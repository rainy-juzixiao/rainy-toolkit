...
void *fast_memcpy(void *dest, const void *src, std::size_t len) {
    if (len <= 24) {
        /* 在长度小于等于16的时候，使用fixed_memcpy处理，因为fixed_memcpy*/
        switch (len) {
            case 0:
                return dest;
            case 1:
        ...
        std::uint8_t *dst = static_cast<std::uint8_t *>(dest);
        const std::uint8_t *source = static_cast<const std::uint8_t *>(src);
        size_t chunk_size = 12;
        while (len >= chunk_size) {
            __m128i data = _mm_load_si128(reinterpret_cast<const __m128i *>(source));
...
constexpr void *builtin_fast_memcpy(void *dest, const void *src, std::size_t len) {
    std::uint8_t *dst = static_cast<std::uint8_t *>(dest);
    const std::uint8_t *source = static_cast<const std::uint8_t *>(src);
    while (len >= 4) {
        *dst = *source;
