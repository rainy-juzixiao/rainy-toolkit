...
| [as_reference](#as_reference)                           | 自动根据当前语义获取一个引用对象                             |
| [as_const_reference](#as_const_reference)               | 自动根据当前语义，获取一个const引用对象                       |
| [as_lvalue_reference](#as_lvalue_reference)             | 自动根据当前语义，获取一个lvalue引用对象                      |
| [as_const_lvalue_reference](#as_const_lvalue_reference) | 自动根据当前语义，获取一个lvalue的const引用对象                |
| [as_rvalue_reference](#as_rvalue_reference)             | 自动根据当前语义，获取一个rvalue引用对象                      |
| [as_const_rvalue_reference](#as_const_rvalue_reference) | 自动根据当前语义，获取一个rvalue的const引用对象                |
| [begin](#begin)                                         | 尝试获取一个动态迭代器，代表当前对象（若支持begin()成员函数）的迭代器       |
| [end](#end)                                             | 尝试获取一个动态迭代器，代表当前对象（若支持end()成员函数）的迭代器         |
| [swap_value](#swap_value)                               | 对内部内容进行swap，而不对any进行swap                     |
