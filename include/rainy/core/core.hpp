...
* @brief 它将建立一个基本的命名空间结构，并提供一下基础函数和元编程工具
...
* @brief 内部实现：rainy::core::internals
...
namespace rainy::core {
    ...
    public:
        ...
        temporary_buffer(Ty *buffer, difference_type count, difference_type capacity) :
            ...
            internals::stl_internal_check(buffer);
        }
        ...
        void reallocate(const std::size_t realloc) {
            // 永远不可能保证线程安全
            if (buffer_) {
                if (realloc <= count_ || realloc == 0 || internals::in_range<std::size_t>(count_, capacity_, realloc)) {
            ...
            return_buffer(); // 需要归还
            internals::stl_internal_check(buffer_);
            buffer_ = new_buffer;
    ...
    temporary_buffer<Ty> get_temporary_buffer(std::ptrdiff_t count) noexcept {
        ...
        return {static_cast<Ty *>(pal::allocate(internals::get_size_of_n<Ty>(new_size), alignof(Ty))), count, new_size};
    }
...
namespace rainy::core {
    RAINY_INLINE bool is_main_thread() noexcept {
        return internals::main_thread_id == std::this_thread::get_id();
    }
...
namespace rainy::component::ranges::container_operater {
    ...
    RAINY_CONSTEXPR20 OutIter copy(Rng &&rng, OutIter dest) noexcept(
        type_traits::type_properties::is_nothrow_copy_constructible_v<typename utility::iterator_traits<OutIter>::value_type>) {
        return algorithm::container_operater::copy(utility::begin(utility::forward<Rng>(rng)), utility::end(utility::forward<Rng>(rng)),
                                                   dest);
    }

    template <typename Rng, typename OutIter>
    RAINY_CONSTEXPR20 OutIter copy_n(Rng &&rng, std::size_t count, OutIter dest) noexcept(
        type_traits::type_properties::is_nothrow_copy_constructible_v<typename utility::iterator_traits<OutIter>::value_type>) {
        return algorithm::container_operater::copy_n(utility::begin(rng), count, dest);
    }
}
