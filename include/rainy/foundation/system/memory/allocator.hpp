...
namespace rainy::foundation::system::memory {
    ...
    public:
        ...
        template <typename U>
        RAINY_CONSTEXPR20 allocator(std::allocator<U>) noexcept {
        }
        ...
        RAINY_NODISCARD_RAW_PTR_ALLOC RAINY_CONSTEXPR20 pointer allocate(const size_type count) const {
            ...
            if constexpr (Method == allocation_method::std_allocator) {
                ...
                rainy_let allocated_memory =
                    static_cast<pointer>(std::malloc(core::internals::get_size_of_n<value_type>(count)));
                if (!allocated_memory) {
                    throw_bad_alloc();
                }
            ...
            rainy_let allocated_memory = static_cast<value_type *>(
                core::pal::allocate(core::internals::get_size_of_n<value_type>(count), align));
            if (!allocated_memory) {
                throw_bad_alloc();
            }
            return allocated_memory;
        ...
        RAINY_CONSTEXPR20 void deallocate(value_type *block, const size_type count) const {
...
#endif
            {
                if constexpr (Method == allocation_method::std_allocator) {
                    ...
                    core::pal::deallocate(block, core::internals::get_size_of_n<value_type>(count), align);
                }
            }
        }
    ...
    namespace pmr {
        ...
        private:
            ...
            void do_deallocate(void *block, std::size_t bytes, std::size_t alignment) override {
                if (bytes == 0 || !core::internals::is_pow_2(alignment) || !block) {
                    return;
                }
                std::free(block);
            }
        ...
        private:
            void *do_allocate(std::size_t bytes, std::size_t alignment) override {
                if (bytes == 0 || !core::internals::is_pow_2(alignment)) {
                    return nullptr;
                }
                return core::pal::allocate(bytes, alignment);
            }
        ...
        public:
            ...
            RAINY_NODISCARD_RAW_PTR_ALLOC value_type *allocate(const size_t count) {
                void *const resource = _resource->allocate(core::internals::get_size_of_n<Ty>(count), align);
                return static_cast<value_type *>(resource);
            }

            void deallocate(Ty *const ptr, const size_t count) noexcept {
                _resource->deallocate(ptr, core::internals::get_size_of_n<Ty>(count), align);
            }
            ...
            RAINY_NODISCARD_RAW_PTR_ALLOC value_type *allocate_object(const size_t count = 1) {
                void *const resource = allocate_bytes(core::internals::get_size_of_n<value_type>(count), align);
                return static_cast<value_type *>(resource);
            }

            void deallocate_object(value_type *const ptr, const size_t count = 1) noexcept {
                deallocate_bytes(ptr, core::internals::get_size_of_n<value_type>(count), align);
            }
