...
namespace rainy::foundation::io::internals {
...
namespace rainy::foundation::io {
     ...
     */
    template <typename... Args, type_traits::other_trans::enable_if_t<internals::is_printable_v<Args...>, int> = 0>
    void stdout_print(Args... args) {
        std::string output_string = internals::make_sstream_string(args...);
    ...
    void ts_stdout_print(Args... args) {
        std::lock_guard<std::shared_mutex> lock(internals::get_mtx());
        stdout_print(args...);
    }
