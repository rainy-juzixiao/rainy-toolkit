...
namespace rainy::containers {
    ...
    public:
        ...
        RAINY_NODISCARD const utility::sti_typeinfo &type() const noexcept {
            // if *this contains a value of type T, return sti_typeid(T); otherwise sti_typeid(void)
            const utility::sti_typeinfo *const info = type_info();
            if (info) {
                return *info;
            }
    ...
    private:
        ...
        RAINY_NODISCARD const utility::sti_typeinfo *type_info() const noexcept { // extract the utility::sti_typeinfo from type_data
            return reinterpret_cast<const utility::sti_typeinfo *>(storage.type_data & ~rep_mask);
        }
