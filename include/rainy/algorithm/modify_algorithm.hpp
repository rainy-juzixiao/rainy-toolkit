...
namespace rainy::component::ranges::container_operater {
    ...
    RAINY_CONSTEXPR20 OutIter copy(Rng &&rng, OutIter dest) noexcept(
        std::is_nothrow_copy_constructible_v<typename utility::iterator_traits<OutIter>::value_type>) {
        return algorithm::container_operater::copy(utility::begin(rng), std::ranges::end(rng), dest);
    }

    template <typename Rng, typename OutIter>
    RAINY_CONSTEXPR20 OutIter copy(algorithm::execution::policy policy, Rng &&rng, OutIter dest) {
        return algorithm::container_operater::copy(policy, utility::begin(rng), std::ranges::end(rng), dest);
    }
