...
namespace rainy::foundation::diagnostics {
    ...
    public:
        using handler_type = functional::function_pointer<void(const utility::source_location &)>;
         ...
         * @return 之前的合约处理函数指针
         */
        RAINY_INLINE static handler_type set_contract_handler(
            const functional::function_pointer<void(const utility::source_location & /* location */)> fptr);
         ...
         * @attention 如果你出现了这些问题，推荐在CMake的配置清单中的COMMON_MOUDLE_USE_SOURCE_BUILD_MODE进行禁用再次尝试编译
         */
        RAINY_INLINE static void invoke_contract_handler(const utility::source_location &location);
...
namespace rainy::foundation::diagnostics::internals {
...
#define rainy_assert(expr, msg) \
    ::rainy::foundation::diagnostics::internals::assert_impl((expr), #expr, msg, ::rainy::utility::source_location::current())
...
#include <rainy_cxxfiles/diagnostics/contract.cxx>
#endif
