...
namespace rainy::foundation::system::memory {
    ...
    public:
        ...
        template <typename U>
        constexpr allocator(allocation_method method) {
        }
        ...
        RAINY_NODISCARD_RAW_PTR_ALLOC RAINY_CONSTEXPR20 pointer allocate(const size_type count) const {
            ...
            if constexpr (Method == allocation_method::std_allocator) {
                ...
                rainy_let allocated_memory =
                    static_cast<pointer>(std::malloc(information::internals::get_size_of_n<value_type>(count)));
                if (!allocated_memory) {
                    throw_bad_alloc();
                }
            ...
            rainy_let allocated_memory = static_cast<value_type *>(
                information::pal::allocate(information::internals::get_size_of_n<value_type>(count), align));
            if (!allocated_memory) {
                throw_bad_alloc();
            }
            return allocated_memory;
        ...
        RAINY_CONSTEXPR20 void deallocate(value_type *block, const size_type count) const {
...
#endif
            {
                if constexpr (Method == allocation_method::std_allocator) {
                    ...
                    information::pal::deallocate(block, information::internals::get_size_of_n<value_type>(count), align);
                }
            }
        }
...
#else
                return information::pal::allocate(bytes, alignment);
#endif
            }
...
#else
                information::pal::deallocate(block, bytes, alignment);
#endif
            }
...
#if RAINY_HAS_CXX20
        template <typename Ty = information::byte_t>
#else
        template <typename Ty>
#endif
