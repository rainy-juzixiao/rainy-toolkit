...
namespace rainy::user::hash::internals::sha512 {
    RAINY_HPP_INLINE void preprocess_message(const std::string &input, std::vector<uint8_t> &message) {
        message.clear();
        // 原始消息转换
        for (char c: input) {
    ...
    RAINY_HPP_INLINE void compress(uint64_t (&state)[8], const std::vector<std::uint8_t> &message_block) {
    ...
    RAINY_HPP_INLINE void preprocess_message_from_file(const std::string_view file_path, std::vector<uint8_t> &message) {
...
namespace rainy::user::hash::internals::sha256 {
    ...
    RAINY_HPP_INLINE void compress(std::uint32_t (&state)[8], const std::vector<uint8_t> &message_block) {
        std::uint32_t w[64];
        for (int i = 0; i < 16; ++i) {
