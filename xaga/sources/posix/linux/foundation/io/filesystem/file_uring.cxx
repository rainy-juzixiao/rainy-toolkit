...
namespace rainy::foundation::io::filesystem::implements {
    static io_uring_sqe *get_sqe_from_op(completion_op *op, net::implements::io_context_impl_base &ctx_impl, int fd) {
    ...
    public:
        ...
        std::error_code open(const std::filesystem::path &path, open_mode mode, net::implements::io_context_impl_base &ctx) noexcept override {
            int flags = 0;
            bool r = has_flag(mode, open_mode::read_only);
            bool w = has_flag(mode, open_mode::write_only);
            ...
            // 将 fd 注册到 io_uring（associate_handle 内部会做 io_uring_register_files 或记录）
        ...
        std::size_t write_some_at(net::const_buffer buf, std::uint64_t offset, std::error_code &ec) noexcept override {
            ssize_t n = ::pwrite(fd_, buf.data(), buf.size(), static_cast<::off_t>(offset));
            if (n < 0) {
                ec.assign(errno, std::system_category());
                return 0;
            }
        ...
        void async_read_some_at(net::mutable_buffer buf, std::uint64_t offset,
                                net::implements::io_context_impl_base &ctx,
            ...
            // 使用 thread_local iovec 或直接使用 prep_read (非 vectored)
            // 对于单缓冲区的读写，可以使用 prep_read 而不是 prep_readv
            ::io_uring_prep_read(sqe, fd_, buf.data(), buf.size(), offset);
            ::io_uring_sqe_set_data(sqe, op);
            submit_ring(op);
        }

        void async_write_some_at(net::const_buffer buf, std::uint64_t offset,
                                 net::implements::io_context_impl_base &ctx,
                                 completion_op *op) noexcept override {
            // 直接使用传入的 op，不创建新的
            io_uring_sqe *sqe = get_sqe_from_op(op, ctx, fd_);
