...
use crate::lang::language::load_easter_eggs;
use rand::Rng;
...
pub async fn code_rain(duration_secs: u64) {
    let code_rain = match load_easter_eggs() {
        Ok(easter_eggs) => easter_eggs.code_rain.first().cloned(),
        Err(err) => panic!("Failed to fetch err message: {}", err),
    }
    .unwrap();
    print!("\n");
    println!("{}", code_rain.init_text);
    println!("{}", code_rain.press_ctrl);
    ...
    while start_time.elapsed() < duration {
        ...
        _ = async {
             for x in 0..width {
                 ...
                 if drops[x] > 0 {
                     ...
                     for y in 0..height {
                         ...
                         if drop_pos >= 0 && drop_pos < drop_len {
                     ...
                     if drops[x] > height + 20 {
                         drops[x] = if rand::thread_rng().gen_range(0..100) < 10 {
                             1
                         } else {
                             0
                         };
             ...
             std::mem::swap(&mut current_buffer, &mut prev_buffer); // 交换缓冲区
    ...
    if interrupted {
        println!("\x1b[33m{}\x1b[0m", code_rain.interrupted_text);
    } else {
        print!("\x1b[2J\x1b[H\x1b[?25h");
        print!("\n\x1b[32m");
        print!("{}", code_rain.complete);
        print!("\x1b[0m\n");
        println!("\x1b[90m {}.\x1b[0m\n", code_rain.welcome_back);
    }
}
