...
int main() {
    object o{0};
    auto name = meta::moon::get_member_names<object>();
    std::cout << name[0] << '\n';
    ...
    //constexpr auto names = meta::moon::get_member_names<object>();
    //constexpr auto ind = meta::moon::index_of<object, "obj">();

    //constexpr auto index = meta::moon::get<"obj">(ooo);
    //meta::moon::for_each<structure>([](std::string_view name, std::size_t idx) { std::cout << name << ' ' << idx << '\n'; });
    {
        using namespace rainy::meta::reflection;
