...
namespace rainy::text::implements {
...
#else
            foundation::exceptions::runtime::throw_runtime_error("Format string and arguments count mismatch");
#endif
        }
        return format_placeholder;
    }
    ...
    // clang-format off
    static constexpr collections::array ba_bytes_sf_tables(
    {
    ...
    static constexpr collections::array ba_bytes_nf_tables(
    {
    ...
    static constexpr collections::array additional_allocation_tables(
    {
    ...
    constexpr bool is_in_range(std::size_t start, std::size_t end, std::size_t value) {
        return core::implements::in_range<std::size_t>(start, end, value);
    }
    ...
    void cstyle_format(std::basic_string<CharType> &ref, const std::basic_string_view<CharType> fmt, Args &&...args) {
        ...
        auto check_func = [&ref](int update_size) {
            if (update_size < 0) {
                ...
                core::pal::debug_break();
#else
                foundation::exceptions::runtime::throw_runtime_error("Error formatting string");
#endif
            }
        };
        ...
        if (const auto format_placeholder = implements::check_format_args(arity, ref, fmt); ref.empty()) {
            possible_size = implements::calc_pre_allocation<arity>(raw_size, format_placeholder.number_format_placeholder_count,
                                                                  format_placeholder.string_format_placeholder_count);
            ...
            if (possible_size > ref_capacity) {
                const std::size_t alloc =
                    implements::calc_allocation<arity>(raw_size, ref_capacity, format_placeholder.number_format_placeholder_count,
                                                      format_placeholder.string_format_placeholder_count);
                ref.reserve(alloc);
                ref.resize(alloc);
            }
        ...
        do { // NOLINT
            if constexpr (type_traits::type_relations::is_same_v<CharType, char>) {
                ...
                update_size = implements::vswprintf_wrapper(&ref[0], ref.capacity(), fmt.data(),
