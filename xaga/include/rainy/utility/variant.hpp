...
namespace rainy::collections {
    template <typename... Types>
    class variant : public implements::variant_base<Types...> {
    public:
        using base = implements::variant_base<Types...>; // 引用，此处忽略
        ...
        constexpr variant(Ty &&object) noexcept(std::is_nothrow_constructible_v<implements::variant_init_type<Ty, Types...>, Ty>) :
            base(utility::placeholder_index<implements::variant_init_index<Ty, Types...>::value>, object) {
        }
    ...
    constexpr void visit(Visitor &&vis, Variants &&...vars) {
        (implements::visit_impl(vis, utility::forward<Variants>(vars)), ...);
    }
}
