...
namespace rainy::meta::reflection::moon {
    ...
    struct reflect_base {
        ...
        static constexpr std::string_view name = foundation::rtti::type_name<type>();
        static constexpr std::size_t size = sizeof(type);
        static constexpr std::size_t align = alignof(type);

        static constexpr std::size_t propertie_count = implements::eval_valid_properties_count<type>();
        ...
        static constexpr auto DFS_Acc(R r, F &&f) {
            return implements::DFS_Acc<0>(
                reflect<type>{}, std::forward<F>(f),
                virtual_bases().Accumulate(std::forward<F>(f)(std::move(r), reflect<type>{}, 0),
