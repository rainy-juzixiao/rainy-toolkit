...
#include <rainy/implements/reflection_core/field.hpp>
#include <rainy/foundation/system/memory/nebula_ptr.hpp>
#include <rainy/implements/reflection_core/method_invoker.hpp>
...
namespace rainy::foundation::reflection {
    ...
    public:
        ...
        auto invoke(void *object = nullptr, const std::vector<utility::any> &args = {}) const {
            if (empty()) {
        ...
        template <typename ReturnType = void>
        auto invoke(const void *object = nullptr, const std::vector<utility::any> &args = {}) const {
            if (empty()) {
                rainy::foundation::system::exceptions::runtime::throw_runtime_error("internal object is error!");
            }
            if constexpr (std::is_void_v<ReturnType>) {
        ...
        auto invoke_with_paramspack(void *object = nullptr, Args &&...args) const {
            ...
            std::vector<utility::any> params_pack = {rainy::utility::forward<Args>(args)...};
            return invoker->invoke(object, params_pack);
        }

        utility::any operator()(void *object, const std::vector<utility::any> &params) const {
    ...
    private:
        struct invoker_helper {
            virtual utility::any invoke(void *,
                                                                 const std::vector<utility::any> &) const = 0;
            virtual utility::any invoke(const void *,
                                                                 const std::vector<utility::any> &) const = 0;
        ...
        public:
            ...
            utility::any invoke(void *object, const std::vector<utility::any> &params) const override {
                ...
                if (cur_type >= method_type::const_method && cur_type <= method_type::const_method_right_volatile_noexcept) {
                    ...
                    if (cur_type >= method_type::const_method && cur_type <= method_type::const_method_volatile_noexcept) {
                        return implements::call_cmethod(storage.get(),cur_type, object, params);
                    } else if (cur_type >= method_type::const_method_left &&
                               cur_type <= method_type::const_method_left_volatile_noexcept) {
                        return implements::call_cmethod_left(storage.get(), cur_type, object, params);
                    } else {
                        return implements::call_cmethod_right(storage.get(), cur_type, object, params);
                    }
                    ...
                    if (cur_type >= method_type::normal_method && cur_type <= method_type::normal_method_left_volatile_noexcept) {
                        return implements::call_normal_method(storage.container, cur_type, object, params);
                    } else if (cur_type >= method_type::normal_method_left &&
                               cur_type <= method_type::normal_method_left_volatile_noexcept) {
                        return implements::call_normal_method_left(storage.get(), cur_type, object, params);
                    } else {
                        return implements::call_normal_method_right(storage.get(), cur_type, object, params);
                    }
                } else {
                    switch (cur_type) {
            ...
            utility::any invoke(const void *object, const std::vector<utility::any> &params) const override {
        ...
        private:
            struct storage_t {
                ...
                implements::union_method_container<Class, ReturnType, Args...> container;
                method_type type;
            };
