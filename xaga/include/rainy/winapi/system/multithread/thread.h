...
namespace rainy::winapi::system::multithread {
     ...
     */
    RAINY_TOOLKIT_API win32_thread_result thread_join(std::uintptr_t thread_handle,
                                                                       std::int64_t *result_receiver) noexcept;

    /**
     * @brief 使线程句柄失效，以防止其被join
     * @attention 调用此函数后，线程将导致未定义行为，不保证其安全性，且也不保证其不发生UB
     */
    RAINY_TOOLKIT_API win32_thread_result thread_detach(std::uintptr_t thread_handle) noexcept;
     ...
     */
    RAINY_TOOLKIT_API void suspend_thread(std::uintptr_t thread_handle) noexcept;
     ...
     * @attention 另外，如果thread_handle的访问权限不足，errno并不会被设置为EINVAL，而是设置为EACCES
     */
    RAINY_TOOLKIT_API void resume_thread(std::uintptr_t thread_handle) noexcept;
}
