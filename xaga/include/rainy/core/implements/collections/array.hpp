...
namespace rainy::collections {
    ...
    public:
         ...
         * @return 返回对应下标的元素引用
         */
        RAINY_NODISCARD RAINY_ALWAYS_INLINE constexpr reference at(difference_type off) {
            range_check(off);
            return elems_[off];
        }
         ...
         * @return 返回对应下标的元素引用
         */
        RAINY_NODISCARD RAINY_ALWAYS_INLINE constexpr const_reference at(difference_type off) const {
         ...
         * @return 返回第一个元素的引用
         */
        RAINY_NODISCARD RAINY_ALWAYS_INLINE constexpr reference front() {
            check_zero_length_array();
            return elems_[0];
        }
         ...
         * @return 返回最后一个元素的引用
         */
        RAINY_NODISCARD RAINY_ALWAYS_INLINE constexpr reference back() {
            check_zero_length_array();
            return elems_[N - 1];
        }
         ...
         * @return 返回对应下标的元素引用
         */
        RAINY_NODISCARD RAINY_ALWAYS_INLINE constexpr reference operator[](difference_type idx) noexcept {
         ...
         */
        RAINY_ALWAYS_INLINE constexpr void swap(array &right) noexcept(std::is_nothrow_swappable_v<Ty>) {
            check_zero_length_array();
            core::algorithm::swap_ranges(elems_, right.elems_);
        }
