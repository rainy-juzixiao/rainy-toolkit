...
namespace rainy::type_traits::implements::gcc_detail_impl {
    ...
    template <typename Ty>
    struct is_function : helper::bool_constant<implements::_is_function_v<Ty>> {};

    struct _do_is_destructible_impl {
        template <typename Ty, typename = decltype(std::declval<Ty &>().~Ty())>
    ...
    template <typename Ty>
    struct is_void : helper::bool_constant<implements::is_same_v<void, Ty>> {};
    ...
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_arithmetic_v = implements::is_integral_v<Ty> || implements::is_floating_point_v<Ty>;

    template <typename Ty>
    struct is_scalar : helper::bool_constant<is_arithmetic_v<Ty> || __is_enum(Ty) || implements::_is_pointer_v<Ty> || std::is_member_pointer_v<Ty> ||
