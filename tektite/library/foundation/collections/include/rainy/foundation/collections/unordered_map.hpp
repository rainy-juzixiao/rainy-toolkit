 ...
 */
#ifndef RAINY_FOUNDATION_COLLECTIONS_UNORDERED_MAP_HPP
#define RAINY_FOUNDATION_COLLECTIONS_UNORDERED_MAP_HPP
#include <cmath>
...
namespace rainy::foundation::collections::implements {
    ...
    public:
        ...
        using value_type = container::pair<const Key, Mapped>;
...
namespace rainy::foundation::collections {
    template <typename Key, typename Mapped, typename Hash = utility::hash<Key>, typename KeyEqual = functional::equal<Key>,
              typename Allocator = core::memory::allocator<container::pair<const Key, Mapped>>>
    ...
    public:
        ...
        template <typename M>
        container::pair<iterator, bool> insert_or_assign(const key_type &key, M &&obj) {
            auto it = this->find(key);
            if (it != this->end()) {
                it->second = utility::forward<M>(obj);
                return container::make_pair(it, false);
            }
            return this->emplace(key, utility::forward<M>(obj));
        }

        template <typename M>
        container::pair<iterator, bool> insert_or_assign(key_type &&key, M &&obj) {
            auto it = this->find(key);
            if (it != this->end()) {
                it->second = utility::forward<M>(obj);
                return container::make_pair(it, false);
        ...
        template <typename... Args>
        container::pair<iterator, bool> try_emplace(const key_type &key, Args &&...args) {
            auto it = this->find(key);
            if (it != this->end()) {
                return container::make_pair(it, false);
            }
            return this->emplace(container::piecewise_construct, container::forward_as_tuple(key),
                                 container::forward_as_tuple(utility::forward<Args>(args)...));
        }

        template <typename... Args>
        container::pair<iterator, bool> try_emplace(key_type &&key, Args &&...args) {
            auto it = this->find(key);
            if (it != this->end()) {
                return container::make_pair(it, false);
            }
            return this->emplace(container::piecewise_construct, container::forward_as_tuple(utility::move(key)),
                                 container::forward_as_tuple(utility::forward<Args>(args)...));
        }
        ...
        mapped_type &at(const key_type &key) {
            auto it = this->find(key);
            if (it == this->end()) {
                core::exceptions::logic::throw_out_of_range("unordered_map::at: key not found");
            }
