 ...
 */
#ifndef RAINY_FOUNDATION_COLLECTIONS_LIST_HPP
#define RAINY_FOUNDATION_COLLECTIONS_LIST_HPP
...
namespace rainy::foundation::collections {
     ...
     */
    template <typename Ty, typename Allocator = core::memory::allocator<Ty>>
    class list {
    public:
        struct node {
            Ty value;
        ...
        list(const list &right, const type_traits::primary_types::type_identity_t<allocator_type> &alloc) :
        ...
        void swap(list &right) noexcept(core::memory::allocator_traits<Allocator>::is_always_equal::value) {
            std::swap(sentinel_, right.sentinel_);
            std::swap(size_, right.size_);
        ...
        void merge(list &right) {
            merge(right, functional::less<Ty>());
        }
