...
namespace rainy::foundation::dynamic_library::implements {
    ...
    constexpr core::text::string_view get_suffixes_sys() {
    ...
    static bool file_exist(const core::text::string_view fileName) {
            ...
            for (rainy_let prefix = 0u; retry && !hand && prefix < prefix_list.size(); ++prefix) {
                for (rainy_let suffix = 0u; retry && !hand && suffix < suffix_list.size(); ++suffix) {
                    if (!prefix_list[prefix].empty() && module_path.starts_with(prefix_list[prefix])) {
                        attempt = core::text::string{module_path.data(), module_path.size()};
                    } else {
                        attempt = prefix_list[prefix] + core::text::string{module_path.data(), module_path.size()};
                    }
                    if (!suffix_list[suffix].empty() &&
                        (module_path.ends_with(suffix_list[suffix]) || module_path.find(".so.") != core::text::string::npos)) {
                        attempt = core::text::string{module_path.data(), module_path.size()};
                    } else {
