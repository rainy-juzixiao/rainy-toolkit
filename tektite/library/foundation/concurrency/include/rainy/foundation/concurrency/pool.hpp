...
namespace rainy::foundation::concurrency {
    ...
    public:
        explicit pooled_actor_pool(const std::size_t thread_count = std::thread::hardware_concurrency(),
            ...
            for (std::size_t i = 0; i < actor_count_; ++i) {
                actors_.emplace_back(core::memory::make_nebula<actor_worker>(i));
            }
            // 给每个 actor 注入完成回调——减全局计数，而不是 per-actor 计数
            for (auto &a: actors_) {
                a->set_on_complete([this] {
                    const int done = complete_count_.fetch_add(1, core::layer::memory_order_acq_rel) + 1;
                    if (const int submitted = submit_count_.load(core::layer::memory_order_acquire); done == submitted) {
            ...
            for (std::size_t i = 0; i < thread_count_; ++i) {
                threads_.emplace_back(
                    core::memory::make_nebula<thread>(thread::policy::auto_join, [this, i] { thread_loop(i); }));
            }
        }
        ...
        void submit(functional::move_only_delegate<void()> task) override {
            submit_count_.fetch_add(1, core::layer::memory_order_relaxed);
            route(utility::move(task));
        }

        void wait_all() override {
            int target = submit_count_.load(core::layer::memory_order_acquire);
            if (target == 0) {
                return;
            }
            unique_lock lk(idle_mutex_);
            idle_cv_.wait(lk, [this, target] { return complete_count_.load(core::layer::memory_order_acquire) >= target; });
        }
        ...
        RAINY_NODISCARD bool stopped() const noexcept override {
            return stop_.load(core::layer::memory_order_acquire);
        }

        void stop() override {
            stop_.store(true, core::layer::memory_order_release);
            for (auto &a: actors_) {
                a->signal_stop();
            }
        }
    ...
    private:
        void route(functional::move_only_delegate<void()> task) {
            std::size_t idx = round_robin_.fetch_add(1, core::layer::memory_order_relaxed) % actor_count_;
        ...
        void thread_loop(const std::size_t thread_idx) {
            ...
            while (!stop_.load(core::layer::memory_order_acquire)) {
                bool did_work = false;
        ...
        core::concurrency::atomic<std::size_t> round_robin_;
...
namespace rainy::foundation::concurrency {
    ...
    private:
        ...
        std::size_t actor_count_;
        core::concurrency::atomic<std::size_t> round_robin_;
        core::concurrency::atomic<int> submit_count_;
        core::concurrency::atomic<int> complete_count_;
...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        void wait_all() override {
            const int target = submit_count_.load(core::layer::memory_order_acquire);
            if (target == 0) {
                return;
            }
            unique_lock lk(idle_mutex_);
    ...
    private:
        void route(functional::move_only_delegate<void()> task) {
            std::size_t idx = round_robin_.fetch_add(1, core::layer::memory_order_relaxed) % thread_count_;
...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        explicit priority_actor_pool(const std::size_t thread_count = std::thread::hardware_concurrency(),
            ...
            for (std::size_t tier = 0; tier < actor_priority_levels; ++tier) {
                ...
                // NOLINTEND
                rr.store(0, core::layer::memory_order_relaxed);
                ...
                for (std::size_t i = 0; i < actors_per_tier_; ++i) {
                    ...
                    tier_actors.emplace_back(core::memory::make_nebula<actor_worker>(global_id));
                }
    ...
    private:
        ...
        void route(functional::move_only_delegate<void()> task, const actor_priority priority) {
            ...
            std::size_t idx = rr.fetch_add(1, core::layer::memory_order_relaxed) % actors_per_tier_;
...
namespace rainy::foundation::concurrency {
    ...
    public:
            ...
            ready_threads_(0) // ← 新增
        {
            ...
            // 等所有线程真正进入 wait 状态
            {
                ...
                queue_cv_.wait(lk, [this] { // 复用 queue_cv_ 即可
                    return ready_threads_.load(core::layer::memory_order_acquire) >= base_threads_;
                });
            }
        }
        ...
        void submit(functional::move_only_delegate<void()> task) override {
            pending_tasks_.fetch_add(1, core::layer::memory_order_relaxed);
            {
                lock_guard lk(queue_mutex_);
        ...
        void wait_all() override {
            ...
            idle_cv_.wait(lk, [this] {
                int p = pending_tasks_.load(core::layer::memory_order_acquire);
                int c = complete_count_.load(core::layer::memory_order_acquire);
                return p == 0 || c >= p;
            });
            pending_tasks_.store(0, core::layer::memory_order_release);
            complete_count_.store(0, core::layer::memory_order_release);
        }
        ...
        RAINY_NODISCARD scheduler_traits traits() const override {
            ...
            traits.concurrency = active_threads_.load(core::layer::memory_order_relaxed);
            return traits;
        }
    ...
    private:
        void spawn_thread() {
            auto thread_ptr = core::memory::make_nebula<thread>(thread::policy::auto_join, [this] { thread_loop(); });
            {
                lock_guard lk(threads_mutex_);
                threads_.push_back(std::move(thread_ptr));
            }
            active_threads_.fetch_add(1, core::layer::memory_order_relaxed);
        }

        void thread_loop() {
            {
                lock_guard lk(queue_mutex_);
                ready_threads_.fetch_add(1, core::layer::memory_order_release);
                queue_cv_.notify_all();
            }
            while (!stop_.load(core::layer::memory_order_acquire)) {
                ...
                bool has_task = false;
                {
                    ...
                    queue_cv_.wait(lk, [this] {
                        return !task_queue_.empty() || stop_.load(core::layer::memory_order_acquire);
                    });
                ...
                if (has_task) {
                    task();
                    const int done = complete_count_.fetch_add(1, core::layer::memory_order_acq_rel) + 1;
                    const int pending = pending_tasks_.load(core::layer::memory_order_acquire);
                    if (done == pending) {
                        lock_guard lk(idle_mutex_);
                        idle_cv_.notify_all();
                    }
        ...
        void check_expand_pool() {
            std::size_t current_active = active_threads_.load(core::layer::memory_order_relaxed);
        ...
        core::concurrency::atomic<std::size_t> active_threads_;
        core::concurrency::atomic<int> pending_tasks_{0};
        core::concurrency::atomic<int> complete_count_{0};
        core::concurrency::atomic<std::size_t> ready_threads_{0};
    };
}
