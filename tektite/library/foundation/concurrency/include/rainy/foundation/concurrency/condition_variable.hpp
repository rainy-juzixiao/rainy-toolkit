...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        condition_variable() {
            ...
            if (const implements::thrd_result result = cnd_create(&cnd_); result != implements::thrd_result::success) {
        ...
        rain_fn wait(unique_lock<mutex> &lock) -> void { // NOLINT
            ...
            if (const implements::thrd_result result = cnd_wait(&cnd_, mtx); result != implements::thrd_result::success) {
        ...
        rain_fn wait_until(unique_lock<mutex> &lock, const std::chrono::time_point<Clock, Duration> &abs_time) -> cv_status {
            ...
            rain_loop {
                ...
                const ::timespec ts = implements::to_timespec(abs_time);
                const implements::thrd_result r = implements::cnd_timedwait(&cnd_, lock.mutex()->backend_handle(), &ts);
                if (r == implements::thrd_result::success) {
                    return cv_status::no_timeout;
                }
                if (r != implements::thrd_result::timed_out) {
...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        condition_variable_any() : mtx_(std::make_shared<mutex>()) {
            ...
            if (const implements::thrd_result r = cnd_create(&cnd_); r != implements::thrd_result::success) {
        ...
        rain_fn wait(Lock &lock) -> void {
            ...
            lock.unlock();
            const implements::thrd_result r = implements::cnd_wait(&cnd_, internal_lk.mutex()->backend_handle());
            internal_lk.unlock();
            lock.lock();
            if (r != implements::thrd_result::success) {
        ...
        rain_fn wait_until(Lock &lock, const std::chrono::time_point<Clock, Duration> &abs_time) -> cv_status {
            ...
            rain_loop {
                ...
                const ::timespec ts = implements::to_timespec(abs_time);
                const implements::thrd_result r = implements::cnd_timedwait(&cnd_, internal_lk.mutex()->backend_handle(), &ts);
                if (r == implements::thrd_result::success) {
                    internal_lk.unlock();
                    lock.lock();
                    return cv_status::no_timeout;
