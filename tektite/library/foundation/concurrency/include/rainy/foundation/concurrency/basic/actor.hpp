...
namespace rainy::foundation::concurrency {
    ...
    private:
            ...
            while (true) {
                ...
                }
                {
                    ...
                    cv_.wait_for(lk, std::chrono::microseconds(100),
                                 [this] { return !local_queue_.empty() || stop_.load(core::layer::memory_order_acquire); });
                }
                if (stop_.load(core::layer::memory_order_acquire) && local_queue_.empty()) {
                    break;
                }
            }
        }
        ...
        core::memory::nebula_ptr<thread> worker_thread_;
    };
}
