...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        bool try_lock() {
            ...
            if (result == implements::thrd_result::success) {
                return true;
            } else if (result == implements::thrd_result::busy) {
                return false;
            } else {
        ...
        bool try_lock_shared() {
            ...
            if (result == implements::thrd_result::busy) {
                return false;
            } else {
    ...
    public:
        ...
        bool try_lock_until(const std::chrono::time_point<Clock, Duration> &abs_time) {
            ...
            if (result == implements::thrd_result::timed_out) {
                return false;
            }
            throw std::system_error(std::make_error_code(std::errc::operation_not_permitted),
        ...
        bool try_lock_shared_until(const std::chrono::time_point<Clock, Duration> &abs_time) {
            ...
            if (result == implements::thrd_result::success) {
                return true;
            } else if (result == implements::thrd_result::timed_out) {
                return false;
            } else {
                throw std::system_error(std::make_error_code(std::errc::operation_not_permitted),
