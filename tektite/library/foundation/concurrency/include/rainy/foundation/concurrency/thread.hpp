...
namespace rainy::foundation::concurrency {
    ...
    public:
        ...
        void start(Fx &&fn, Args &&...args) {
            ...
            auto decay_copied = core::memory::make_nebula<tuple>(utility::forward<Fx>(fn), utility::forward<Args>(args)...);
            constexpr auto invoker = get_invoke_function_addr<tuple>(std::make_index_sequence<1 + sizeof...(Args)>{});
            thread_handle = implements::create_thread(invoker, 0, decay_copied.get());
            if (thread_handle.handle) {
    ...
    private:
        ...
        static unsigned int invoke_function(void *arg_list) {
            core::memory::nebula_ptr<Tuple> fn_vals(static_cast<Tuple *>(arg_list));
...
namespace rainy::foundation::concurrency::this_thread {
    ...
    RAINY_NODISCARD RAINY_TOOLKIT_API ::pthread_t get_current_thread() noexcept;
#else
    RAINY_NODISCARD RAINY_TOOLKIT_API void *get_current_thread() noexcept;
#endif

    RAINY_NODISCARD RAINY_TOOLKIT_API concurrency::thread::id get_id() noexcept;
...
#if !RAINY_HAS_CXX20
    RAINY_NODISCARD RAINY_TOOLKIT_API bool operator!=(thread::id left, thread::id right) noexcept;
    RAINY_NODISCARD RAINY_TOOLKIT_API bool operator<(thread::id left, thread::id right) noexcept;
    RAINY_NODISCARD RAINY_TOOLKIT_API bool operator<=(thread::id left, thread::id right) noexcept;
    RAINY_NODISCARD RAINY_TOOLKIT_API bool operator>(thread::id left, thread::id right) noexcept;
    RAINY_NODISCARD RAINY_TOOLKIT_API bool operator>=(thread::id left, thread::id right) noexcept;
