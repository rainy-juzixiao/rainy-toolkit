...
namespace rainy::foundation::concurrency {
    mutex::mutex() noexcept {
        ...
        if (rainy_const r = implements::mtx_create(&mtx_, implements::mutex_types::try_mtx | implements::mutex_types::plain_mtx);
            r != implements::thrd_result::success) {
            std::terminate();
        }
#else
        if (rainy_const r = implements::mtx_create(&mtx_, implements::mutex_types::try_mtx); r != implements::thrd_result::success) {
            std::terminate();
        }
#endif
    }
    ...
    rain_fn mutex::lock() -> void {
        if (rainy_const r = implements::mtx_lock(&mtx_); r != implements::thrd_result::success) {
            std::terminate();
        }
    }
    ...
    rain_fn mutex::try_lock() noexcept -> bool {
        ...
        if (r == implements::thrd_result::success) {
            return true;
        }
        if (r == implements::thrd_result::busy) {
            return false;
        }
    ...
    rain_fn mutex::unlock() -> void { // NOLINT
        if (rainy_const r = implements::mtx_unlock(&mtx_); r != implements::thrd_result::success) {
            std::terminate();
        }
    }
...
namespace rainy::foundation::concurrency {
    recursive_mutex::recursive_mutex() noexcept {
        ...
        if (const auto r = implements::mtx_create(&mtx_, flags); r != implements::thrd_result::success) {
            std::terminate();
        }
    }
    ...
    recursive_mutex::~recursive_mutex() {
        ...
        if (const auto r = implements::mtx_destroy(&mtx_); r != implements::thrd_result::success) {
            std::terminate();
        }
    }

    rain_fn recursive_mutex::lock() -> void {
        if (const auto r = implements::mtx_lock(&mtx_); r != implements::thrd_result::success) {
            std::terminate();
        }
    }
    ...
    rain_fn recursive_mutex::try_lock() noexcept -> bool {
        switch (const auto r = implements::mtx_trylock(&mtx_); r) {
            case implements::thrd_result::success:
                return true;
    ...
    rain_fn recursive_mutex::unlock() -> void { // NOLINT
        if (const auto r = implements::mtx_unlock(&mtx_); r != implements::thrd_result::success) {
            std::terminate();
        }
    }
...
namespace rainy::foundation::concurrency {
    timed_mutex::timed_mutex() noexcept {
        if (constexpr int flags = implements::mutex_types::try_mtx | implements::mutex_types::timed_mtx;
            implements::mtx_create(&mtx_, flags) != implements::thrd_result::success) {
            std::terminate();
        }
    }
    ...
    timed_mutex::~timed_mutex() {
        ...
        if (implements::mtx_destroy(&mtx_) != implements::thrd_result::success) {
            std::terminate();
        }
    }

    rain_fn timed_mutex::lock() -> void {
        if (implements::mtx_lock(&mtx_) != implements::thrd_result::success) {
            std::terminate();
        }
    }
    ...
    rain_fn timed_mutex::unlock() -> void { // NOLINT
        if (implements::mtx_unlock(&mtx_) != implements::thrd_result::success) {
            std::terminate();
        }
    }
...
namespace rainy::foundation::concurrency {
    recursive_timed_mutex::recursive_timed_mutex() noexcept {
        ...
        if (implements::mtx_create(&mtx_, flags) != implements::thrd_result::success) {
            std::terminate();
        }
    }
