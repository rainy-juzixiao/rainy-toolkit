    ...
    struct Name {                                                                                                                     \
        ...
        struct mixin : Base {                                                                                                         \
            ...
            decltype(auto) member(Args &&...args) {                                                                                   \
                return rainy::core::poly_call<I>(*this, ::rainy::utility::forward<Args>(args)...);                                                 \
            }                                                                                                                         \
            template <typename... Args>                                                                                               \
...
namespace rainy::foundation::poly::implements {
    ...
    struct conv_count;
    template <typename... Cs>
    struct conv_count<convention_list<Cs...>> : type_traits::helper::integral_constant<std::size_t, sizeof...(Cs)> {};
    ...
    template <typename ConvList>
    using index_conventions_t = typename index_conventions_impl<std::make_index_sequence<conv_count<ConvList>::value>, ConvList>::type;
    ...
    struct build_value_list<convention_list<ICs...>, Ty> {
        ...
        static constexpr auto slot() noexcept {
            if constexpr (type_traits::type_relations::is_base_of_v<rainy::core::implements::poly_inspector, Ty>) {
    ...
    constexpr lifecycle_ops make_lifecycle_ops() noexcept {
        return {[](void *s, bool) noexcept {
                ...
                [](void *dst, void *src, bool) noexcept {
                    if constexpr (InSoo) {
                        ::new (dst) Ty(utility::move(*static_cast<Ty *>(src)));
                ...
                []() -> void *(*) (const void *, bool) {
                    if constexpr (type_traits::properties::is_copy_constructible_v<Ty>) {
    ...
    struct support_clone_trait : type_traits::helper::false_type {};
    template <typename F>
    struct support_clone_trait<F, type_traits::other_trans::void_t<decltype(F::support_clone)>> : type_traits::helper::bool_constant<F::support_clone> {};
}

namespace rainy::foundation::poly {
    using facade_builder = implements::builder_state<implements::convention_list<>, false>;
    ...
    public:
        ...
        static constexpr bool cloneable = implements::support_clone_trait<F>::value;

        box() noexcept = default;

        template <typename Ty, typename D = type_traits::other_trans::decay_t<Ty>, typename = type_traits::other_trans::enable_if_t<!type_traits::type_relations::is_same_v<D, box>>>
        explicit box(Ty &&val) {
            emplace<D>(utility::forward<Ty>(val));
        }
        ...
        Ty &emplace(Args &&...args) {
            ...
            if constexpr (soo) {
                Ty *p = ::new (buf()) Ty(utility::forward<Args>(args)...);
                poly_.reset(p);
                ops_ = &implements::lifecycle_for<Ty, true>;
                is_soo_ = true;
                return *p;
            } else {
                Ty *p = new Ty(utility::forward<Args>(args)...);
                std::memcpy(buf(), &p, sizeof(Ty *));
                poly_.reset(p);
                ops_ = &implements::lifecycle_for<Ty, false>;
                is_soo_ = false;
                return *p;
            }
        }
        ...
        template <bool C = cloneable, type_traits::other_trans::enable_if_t<C, int> = 0>
        box clone() const {
            box result;
            if (!ops_ || !ops_->clone) {
                return result;
    ...
    private:
        ...
        alignas(std::max_align_t) collections::array<char, SooSize> storage_{};
        poly_t poly_{};
        const implements::lifecycle_ops *ops_{nullptr};
        bool is_soo_{false};
    };
}
#endif
