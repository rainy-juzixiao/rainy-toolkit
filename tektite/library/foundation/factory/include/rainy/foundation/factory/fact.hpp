 ...
 */
#ifndef RAINY_FOUNDATION_FACTORY_FACT_HPP
#define RAINY_FOUNDATION_FACTORY_FACT_HPP
...
namespace rainy::foundation::fact {
    template <typename Prod, template <typename Key, typename Mapped, typename...> typename Map,
              ...
              template <typename Product, typename...> typename ProjHolder = core::memory::nebula_ptr, typename Fx = Prod *(),
              template <typename FxType, typename...> typename Creater = functional::delegate,
              template <typename FactoryStorageType> typename Alloc = core::memory::allocator>
    ...
    public:
        using impl_t = unsynchronized_factory<Prod, Map, ProdKey, ProjHolder, Fx, Creater, Alloc>;
        ...
        synchronized_factory &emplace(const product_key &id, Producer &&producer) {
            concurrency::lock_guard<concurrency::mutex> lock{mtx_};
...
namespace rainy::foundation::fact {
    ...
    template <typename Prod, typename ProdKey, template <typename Product, typename...> typename ProjHolder, typename Fx,
              template <typename FxType, typename...> typename Creater,
              template <typename Key, typename Mapped, typename...> typename Map,
              template <typename FactoryStorageType> typename Alloc>
    struct factory_traits<unsynchronized_factory<Prod, Map, ProdKey, ProjHolder, Fx, Creater, Alloc>>
        : type_traits::helper::true_type,
          implements::factory_traits_impl<unsynchronized_factory<Prod, Map, ProdKey, ProjHolder, Fx, Creater, Alloc>> {};

    template <typename Prod, typename ProdKey, template <typename Product, typename...> typename ProjHolder, typename Fx,
              template <typename FxType, typename...> typename Creater,
              template <typename Key, typename Mapped, typename...> typename Map,
              template <typename FactoryStorageType> typename Alloc>
    struct factory_traits<synchronized_factory<Prod, Map, ProdKey, ProjHolder, Fx, Creater, Alloc>>
        : type_traits::helper::true_type,
          implements::factory_traits_impl<synchronized_factory<Prod, Map, ProdKey, ProjHolder, Fx, Creater, Alloc>> {};
