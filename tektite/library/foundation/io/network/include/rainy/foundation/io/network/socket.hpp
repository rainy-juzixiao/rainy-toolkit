...
#include <rainy/foundation/io/network/implements/sock.hpp>
...
namespace rainy::foundation::io::net {
    ...
    public:
        ...
        auto async_connect(const endpoint_type &ep, CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code)>::return_type { // NOLINT
        ...
        auto async_wait(wait_type w, CompletionToken &&token) ->
            ...
            auto *op = io::implements::make_executor_completion_op(
                ...
                executor_);
            impl_->async_wait(static_cast<implements::wait_type>(w), executor_.context().get_executor(), op);
            return init.result.get();
        }
    ...
    protected:
        ...
        core::memory::nebula_ptr<implements::socket_impl_base> impl_;
    };
    ...
    public:
        ...
        auto async_receive(const MutableBufferSequence &buffers, CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, std::size_t)>::return_type { // NOLINT
        ...
        auto async_receive(const MutableBufferSequence &buffers, socket_base::message_flags flags, CompletionToken &&token) ->
            ...
            auto *op = io::implements::make_executor_completion_op(
                ...
                this->executor_);
            this->impl_->async_receive(mb.data(), mb.size(), flags, this->executor_, op);
            return init.result.get();
        }
        ...
        auto async_receive_from(const MutableBufferSequence &buffers, endpoint_type &sender, socket_base::message_flags flags,
            ...
            auto *op = io::implements::make_executor_completion_op(
                [this, mb, flags, &sender, raw_sender, handler]() mutable {
                    std::error_code ec;
                ...
                this->executor_);
            this->impl_->async_receive_from(mb.data(), mb.size(), flags, *raw_sender, this->executor_, op);
            return init.result.get();
        }
        ...
        auto async_send_to(const ConstBufferSequence &buffers, const endpoint_type &recipient, socket_base::message_flags flags,
            ...
            auto *op = io::implements::make_executor_completion_op(
                [this, cb, flags, raw_dest, handler]() mutable {
                    std::error_code ec;
                    auto n = this->impl_->send_to(cb.data(), cb.size(), flags, raw_dest, ec);
                    handler(ec, n < 0 ? 0 : static_cast<std::size_t>(n));
                },
                this->executor_);
            this->impl_->async_send_to(cb.data(), cb.size(), flags, raw_dest, this->executor_, op);
            return init.result.get();
        }
    };
    ...
    public:
        ...
        basic_socket_acceptor(io_context &ctx, const endpoint_type &ep, const bool reuse_addr = true) :
            ...
            open(ep.protocol(), ec);
            if (ec) {
                core::exceptions::runtime::throw_system_error(ec, "acceptor::open");
            }
            if (reuse_addr) {
                int opt = 1;
            ...
            ec = impl_->bind(ep.to_raw());
            if (ec) {
                core::exceptions::runtime::throw_system_error(ec, "acceptor::bind");
            }
            ec = impl_->listen(max_listen_connections);
            if (ec) {
                core::exceptions::runtime::throw_system_error(ec, "acceptor::listen");
            }
        }
        ...
        auto async_accept(CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, socket_type)>::return_type { // NOLINTs
            using token_t = type_traits::other_trans::decay_t<CompletionToken>;
        ...
        auto async_accept(io_context &ctx, CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, socket_type)>::return_type { // NOLINT
            using token_t = type_traits::other_trans::decay_t<CompletionToken>;
            async_completion<token_t, void(std::error_code, socket_type)> init(token);
            auto handler = utility::move(init.completion_handler);
            auto raw_peer = std::make_shared<implements::raw_endpoint>();
            auto *op = io::implements::make_executor_completion_op(
                [this, &ctx, raw_peer, handler]() mutable {
                    std::error_code ec;
        ...
        auto async_wait(wait_type w, CompletionToken &&token) ->
            ...
            async_completion<token_t, void(std::error_code)> init(token);
            auto handler = utility::move(init.completion_handler);
            auto *op = io::implements::make_executor_completion_op([handler]() mutable { handler(std::error_code{}); });
