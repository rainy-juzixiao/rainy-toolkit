...
namespace rainy::foundation::io::net::ip {
    ...
    template <typename CharType, typename Traits, typename InternetProtocol>
    std::basic_ostream<CharType, Traits> &operator<<(std::basic_ostream<CharType, Traits> &os, const basic_endpoint<InternetProtocol> &ep) {
        if (ep.address().is_v6()) {
    ...
    public:
        ...
        using const_iterator = typename core::collections::vector<value_type>::const_iterator;
    ...
    private:
        core::collections::vector<value_type> entries_;
    };
    ...
    public:
        ...
        void cancel() {
            cancelled_.store(true, core::concurrency::memory_order_release);
        }
        ...
        auto async_resolve(text::string_view host_name, text::string_view service_name, CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, results_type)>::return_type {
            return async_resolve(protocol_type::v4(), host_name, service_name, 0, utility::forward<CompletionToken>(token));
        }

        template <typename CompletionToken>
        auto async_resolve(text::string_view host_name, text::string_view service_name, flags f, CompletionToken &&token) ->
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, results_type)>::return_type {
            return async_resolve(protocol_type::v4(), host_name, service_name, f, utility::forward<CompletionToken>(token));
        }
        ...
        auto async_resolve(const protocol_type &protocol, text::string_view host_name, text::string_view service_name,
            ...
            typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, results_type)>::return_type {
            return async_resolve(protocol, host_name, service_name, 0, utility::forward<CompletionToken>(token));
        }
    ...
    private:
        results_type do_resolve(const protocol_type &protocol, text::string_view host_name, text::string_view service_name, flags f,
                                std::error_code &ec) {
            if (cancelled_.load(core::concurrency::memory_order_acquire)) {
                ec = std::make_error_code(std::errc::operation_canceled);
                return {};
            }
            text::string host{host_name.data(), host_name.size()};
            text::string svc{service_name.data(), service_name.size()};
            core::collections::vector<implements::resolved_entry> raw_results;
        ...
        core::concurrency::atomic<bool> cancelled_{false};
    };
}
