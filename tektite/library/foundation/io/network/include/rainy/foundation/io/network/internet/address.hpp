...
#include <rainy/foundation/io/network/implements/addr.hpp>

namespace rainy::foundation::exceptions::io {
    class bad_address_cast : public core::exceptions::runtime::runtime_error {
    public:
        bad_address_cast(const source &location) : runtime_error("bad address cast", location) { // NOLINT
        }
    };

    RAINY_INLINE rain_fn throw_bad_address_cast(const core::diagnostics::source_location &location = core::diagnostics::source_location::current())
        -> void {
...
namespace rainy::foundation::io::net::ip {
    ...
    public:
        ...
        struct bytes_type : core::collections::array<unsigned char, 4> {
...
namespace rainy::foundation::io::net::ip {
    class address_v6 {
    public:
        struct bytes_type : core::collections::array<unsigned char, 16> {
            template <typename... Ty>
            explicit constexpr bytes_type(Ty... t) : array{std::in_place, static_cast<unsigned char>(t)...} {
            }
        };
        ...
        RAINY_NODISCARD text::basic_string<char, text::char_traits<char>, Allocator> to_string(const Allocator &alloc = Allocator()) const {
            ...
            if (scope_id_ != 0) {
                char scope_buf[12]{};
                auto [ptr, ec] = core::text::to_chars(scope_buf, scope_buf + sizeof(scope_buf), scope_id_);
                result += '%';
...
namespace rainy::foundation::io::net::ip {
    ...
    std::basic_ostream<CharType, Traits> &operator<<(std::basic_ostream<CharType, Traits> &os, const address &right) {
        return os << right.to_string().c_str();
    }

    template <class CharType, class Traits>
    std::basic_ostream<CharType, Traits> &operator<<(std::basic_ostream<CharType, Traits> &os, const address_v4 &right) {
        return os << right.to_string().c_str();
    }

    template <class CharType, class Traits>
    std::basic_ostream<CharType, Traits> &operator<<(std::basic_ostream<CharType, Traits> &os, const address_v6 &right) {
        return os << right.to_string().c_str();
    }
}
