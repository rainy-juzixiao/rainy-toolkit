...
namespace rainy::foundation::io::net::ip {
    ...
    bool sv_to_cstr(const core::text::string_view sv, char (&buf)[N]) noexcept {
        if (sv.size() >= N) {
            return false;
        }
    ...
    address_v4 make_address_v4(const core::text::string_view sv, std::error_code &ec) noexcept {
        char buf[16]{};
    ...
    address_v4 make_address_v4(const core::text::string_view sv) {
        std::error_code ec;
    ...
    address_v6 make_address_v6(const core::text::string_view sv, std::error_code &ec) noexcept { // NOLINT
        scope_id_type scope = 0;
        core::text::string_view addr_part = sv;
        if (const auto percent = sv.find('%'); percent != core::text::string_view::npos) {
            const auto scope_sv = sv.substr(percent + 1);
            addr_part = sv.substr(0, percent);
            scope_id_type val = 0;
            if (const auto [ptr, e] = core::text::from_chars(scope_sv.data(), scope_sv.data() + scope_sv.size(), val);
    ...
    address_v6 make_address_v6(const core::text::string_view sv) {
        std::error_code ec;
    ...
    address make_address(const core::text::string_view addr, std::error_code &ec) noexcept {
    ...
    address make_address(const core::text::string_view addr) {
        std::error_code ec;
