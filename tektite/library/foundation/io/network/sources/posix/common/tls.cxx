...
namespace rainy::foundation::io::net::implements {
    ...
    public:
        ...
        std::error_code set_alpn_protos(const core::collections::vector<text::string> &protos) noexcept override { // NOLINT
            alpn_protos_ = protos;
        ...
        std::error_code use_certificate_file(const char *path, const char * /*password*/) noexcept override { // NOLINT
            ...
            if (!core::layer::exists_native(cert_file_.c_str())) {
    ...
    public:
        ...
        void async_handshake(io_context::executor_type executor, completion_op *op) noexcept override {
            if (!ssl_ || !op) {
                if (op) {
                    op->complete(io::implements::op_result{nullptr, 0, ENOTCONN}, false);
                }
                return;
            }
            ...
            if (const int err = SSL_get_error(ssl_, ret); err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                ...
                {
                    char probe = 0;
                    const ssize_t r = ::recv(socket_fd_, &probe, 1, MSG_PEEK | MSG_DONTWAIT);
                    if (r == 0 || (r < 0 && errno != EAGAIN && errno != EWOULDBLOCK)) {
                        delete wait_op;
                        op->complete(io::implements::op_result{nullptr, 0, ECONNRESET}, false);
                        return;
                    }
                }
            ...
            static void do_complete(completion_op *self, const io::implements::op_result &,
                ...
                switch (me->op_type_) {
                    ...
                    case ssl_operation_type::shutdown:
                        ...
                        break;
                    default:
                        me->user_op_->complete(io::implements::op_result{nullptr, 0, EINVAL}, false);
                        break;
                }
                delete me;
            }
        ...
        public:
            ...
            static void do_complete(completion_op *self, const io::implements::op_result &, const bool cancelled) noexcept {
                ...
                if (me->op_type_ == ssl_operation_type::write) {
                    ...
                    me->executor_.on_work_finished(); // 未知操作，回滚
                }
