...
namespace rainy::foundation::io::ipc::stream::implements {
    ...
    public:
        ...
        std::error_code attach(native_handle_type handle) noexcept override {
            ...
            fd_ = static_cast<int>(handle);
            cancel_flag_.store(false, core::concurrency::memory_order_relaxed);
            return {};
        }
        ...
        std::error_code close() noexcept override {
            ...
            cancel_flag_.store(true, core::concurrency::memory_order_release);
        ...
        void async_read_some(void *buf, std::size_t len, executor_type executor, completion_op *op) noexcept override {
            ...
            wants_read_ = true;
            int fd = fd_;
            core::concurrency::atomic<bool> *flag = &cancel_flag_;
            get_executor().submit([fd, buf, len, op, flag]() mutable {
                op_result result{};
                result.user_data = op;
                if (flag->load(core::concurrency::memory_order_acquire)) {
        ...
        void reset_operation() noexcept override {
            wants_read_ = false;
            wants_write_ = false;
            cancel_flag_.store(false, core::concurrency::memory_order_release);
        }

    private:
        int fd_{-1};
        executor_type executor_;
        core::concurrency::atomic<bool> cancel_flag_{false};
        bool wants_read_{false};
        bool wants_write_{false};
    };
    ...
    core::memory::nebula_ptr<descriptor_impl_base> create_descriptor_impl(executor_type executor) {
        return core::memory::make_nebula<platform_descriptor_impl>(executor);
    }
    ...
    core::memory::nebula_ptr<descriptor_impl_base> create_descriptor_impl_from_native(executor_type executor, native_handle_type handle) {
        auto impl = core::memory::make_nebula<platform_descriptor_impl>(executor);
    ...
    container::pair<core::memory::nebula_ptr<descriptor_impl_base>, core::memory::nebula_ptr<descriptor_impl_base>> create_pipe_impl(
        ...
        auto read_impl = core::memory::make_nebula<platform_anon_pipe_impl>(executor);
        auto write_impl = core::memory::make_nebula<platform_anon_pipe_impl>(executor);
