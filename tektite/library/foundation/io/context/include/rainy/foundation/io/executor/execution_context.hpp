...
namespace rainy::foundation::exceptions::io {
    class service_already_exists : public core::exceptions::logic::logic_error {
    public:
        service_already_exists(const char *message, const source &location) : logic_error(message, location) {
        }
        service_already_exists(const core::text::string &message, const source &location) : logic_error(message.data(), location) {
        }
    };

    RAINY_INLINE void throw_service_already_exists(
        const core::text::string &message, const core::diagnostics::source_location &location = core::diagnostics::source_location::current()) {
    ...
    public:
        ...
        public:
            struct key {
                ...
                const core::typeinfo *type_info_{nullptr};
