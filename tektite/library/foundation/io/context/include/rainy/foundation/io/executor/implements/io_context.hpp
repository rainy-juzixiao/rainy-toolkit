...
namespace rainy::foundation::io::implements {
    ...
    public:
        template <typename Fx,
                  type_traits::other_trans::enable_if_t<type_traits::properties::is_constructible_v<Func, Fx &&>, int> = 0>
        explicit immediate_op(Fx &&func) noexcept(type_traits::properties::is_nothrow_constructible_v<Func, Fx &&>) :
    ...
    template <typename Func>
    RAINY_NODISCARD immediate_op<type_traits::other_trans::decay_t<Func>> *make_immediate_op(Func &&func) {
        return new immediate_op<type_traits::other_trans::decay_t<Func>>(utility::forward<Func>(func));
    }
    ...
    public:
        ...
        explicit executor_completion_op(Fx &&func, Executor executor) noexcept(
            type_traits::properties::is_nothrow_constructible_v<Func, Fx &&>) :
    ...
    template <typename Func, typename Executor>
    RAINY_NODISCARD executor_completion_op<type_traits::other_trans::decay_t<Func>, Executor> *make_executor_completion_op(Func &&func, Executor executor) {
        return new executor_completion_op<type_traits::other_trans::decay_t<Func>, Executor>(utility::forward<Func>(func), executor);
    }
    ...
    public:
         ...
         */
        virtual core::layer::thrd_result init(int concurrency_hint) noexcept = 0;
         ...
         */
        virtual core::layer::thrd_result associate_handle(completion_op *op, std::uintptr_t fd, void *extra) noexcept = 0;
    ...
    protected:
        core::concurrency::atomic<long> work_count_{0};
        core::concurrency::atomic<bool> stopped_{false};
    };

    RAINY_TOOLKIT_API core::memory::nebula_ptr<io_context_impl_base> create_io_context_impl(int concurrency_hint);
}
