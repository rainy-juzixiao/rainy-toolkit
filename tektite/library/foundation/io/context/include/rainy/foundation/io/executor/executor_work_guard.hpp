...
namespace rainy::foundation::io {
    ...
    template <typename Ty, typename ExecutionContext,
              type_traits::other_trans::enable_if_t<
                  !is_executor<Ty>::value && !type_traits::type_relations::is_convertible_v<Ty &, execution_context &> &&
                      type_traits::type_relations::is_convertible_v<ExecutionContext &, execution_context &>,
                  int> = 0>
    rain_fn make_work_guard(const Ty &t, ExecutionContext &ctx)
        -> executor_work_guard<typename associated_executor<Ty, typename ExecutionContext::executor_type>::type> {
        return executor_work_guard<typename associated_executor<Ty, typename ExecutionContext::executor_type>::type>(
            associated_executor<Ty, typename ExecutionContext::executor_type>::get(t, ctx.get_executor()));
    }
}
