...
namespace rainy::foundation::io {
    ...
    mutable_buffer buffer(core::text::basic_string<CharType, Traits, Allocator> &data) noexcept {
        return {data.data(), data.size() * sizeof(CharType)};
    }

    template <typename CharType, typename Traits, typename Allocator>
    const_buffer buffer(const core::text::basic_string<CharType, Traits, Allocator> &data) noexcept {
        return {data.data(), data.size() * sizeof(CharType)};
    }

    template <typename CharType, typename Traits>
    const_buffer buffer(core::text::basic_string_view<CharType, Traits> data) noexcept {
        return {data.data(), data.size() * sizeof(CharType)};
    }
    ...
    template <typename CharType, typename Traits>
    const_buffer buffer(core::text::basic_string_view<CharType, Traits> data, std::size_t count) noexcept {
...
namespace rainy::foundation::io {
    ...
    public:
        ...
        explicit dynamic_string_buffer(core::text::basic_string<CharType, Traits, Allocator> &str) noexcept :
            str_(str), size_(str.size()), max_size_(str.max_size()) {
        }

        dynamic_string_buffer(core::text::basic_string<CharType, Traits, Allocator> &str, std::size_t maximum_size) noexcept :
    ...
    private:
        core::text::basic_string<CharType, Traits, Allocator> &str_;
        std::size_t size_;
        const std::size_t max_size_;
    };
}
...
namespace rainy::foundation::io {
    ...
    dynamic_string_buffer<CharType, Traits, Allocator> dynamic_buffer(
        core::text::basic_string<CharType, Traits, Allocator> &str) noexcept {
        return dynamic_string_buffer<CharType, Traits, Allocator>{str};
    }

    template <typename CharType, typename Traits, typename Allocator>
    dynamic_string_buffer<CharType, Traits, Allocator> dynamic_buffer(core::text::basic_string<CharType, Traits, Allocator> &str,
...
namespace rainy::foundation::io::implements {
    ...
    std::size_t read_until_impl(SyncReadStream &s, DynamicBuffer &&b, Delim delim, std::error_code &ec) {
        ...
        rain_loop {
            ...
            if constexpr (type_traits::type_relations::is_same_v<Delim, char>) {
                ...
                // string_view 分隔符
                core::text::string_view sv{begin + search_start, static_cast<std::size_t>(end - begin - search_start)};
                auto idx = sv.find(delim);
                if (idx != core::text::string_view::npos) {
...
namespace rainy::foundation::io {
    ...
    template <typename SyncReadStream, typename DynamicBuffer,
              type_traits::other_trans::enable_if_t<is_dynamic_buffer_v<DynamicBuffer>, int> = 0>
    std::size_t read_until(SyncReadStream &s, DynamicBuffer &&b, core::text::string_view delim, std::error_code &ec) {
        return implements::read_until_impl(s, utility::forward<DynamicBuffer>(b), delim, ec);
    }

    template <typename SyncReadStream, typename DynamicBuffer,
              type_traits::other_trans::enable_if_t<is_dynamic_buffer_v<DynamicBuffer>, int> = 0>
    std::size_t read_until(SyncReadStream &s, DynamicBuffer &&b, core::text::string_view delim) {
        std::error_code ec;
    ...
    template <typename AsyncReadStream, typename DynamicBuffer, typename CompletionToken,
              type_traits::other_trans::enable_if_t<is_dynamic_buffer_v<DynamicBuffer>, int> = 0>
    rain_fn async_read_until(AsyncReadStream &s, DynamicBuffer &&b, core::text::string_view delim, CompletionToken &&token) ->
        typename async_result<type_traits::other_trans::decay_t<CompletionToken>, void(std::error_code, std::size_t)>::return_type {
        using token_t = type_traits::other_trans::decay_t<CompletionToken>;
        async_completion<token_t, void(std::error_code, std::size_t)> init(token);
        auto handler = utility::move(init.completion_handler);
        auto buf = utility::forward<DynamicBuffer>(b);
        auto delim_s = core::text::string{delim};
        s.get_executor().post(
