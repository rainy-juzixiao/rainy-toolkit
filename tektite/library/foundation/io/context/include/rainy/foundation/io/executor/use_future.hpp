...
namespace rainy::foundation::io::net {
    template <typename ProtoAllocator = memory::allocator<void>>
    class use_future_t {
    public:
    ...
    private:
        struct std_allocator_void {
            ...
            operator memory::allocator<void>() const { // NOLINT
                return memory::allocator<void>();
            }
        };

        using member_alloc_t =
            type_traits::other_trans::conditional_t<type_traits::type_relations::is_same_v<memory::allocator<void>, allocator_type>,
    ...
    template <typename ProtoAllocator, class Result, typename... Args>
    ...
    template <class Result, typename... Args, class Signature>
