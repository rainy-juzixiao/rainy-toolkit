...
namespace rainy::foundation::io::implements {
    ...
    public:
        ...
        core::layer::thrd_result init(int concurrency_hint) noexcept override {
        ...
        void destroy() noexcept override {
            ...
            stopped_.store(true, core::layer::memory_order_release);
            int empty_count = 0;
            while (empty_count < 2) {
                DWORD bytes = 0;
                ULONG_PTR key = 0;
        ...
        void on_work_started() noexcept override {
            work_count_.fetch_add(1, core::layer::memory_order_relaxed);
        }

        void on_work_finished() noexcept override {
            if (work_count_.fetch_sub(1, core::layer::memory_order_acq_rel) == 1) {
                // 工作归零，唤醒所有阻塞中的 run()
        ...
        void post_immediate_completion(completion_op *op, bool) noexcept override {
            work_count_.fetch_add(1, core::layer::memory_order_release);
            ::PostQueuedCompletionStatus(iocp_handle_, 0, COMPLETION_KEY_IMMEDIATE, reinterpret_cast<OVERLAPPED *>(op));
        }

        core::layer::thrd_result associate_handle(completion_op * /*op*/, std::uintptr_t fd, void * /*extra*/) noexcept override {
            HANDLE h = reinterpret_cast<HANDLE>(fd);
            HANDLE result = ::CreateIoCompletionPort(h, iocp_handle_, COMPLETION_KEY_IO, 0);
            return result != nullptr ? core::layer::thrd_result::success : core::layer::thrd_result::error;
        }

        void stop() noexcept override {
            if (!stopped_.exchange(true, core::layer::memory_order_acq_rel)) {
                int wakeup_count = (concurrency_hint_ > 0) ? concurrency_hint_ : 16;
                for (int i = 0; i < wakeup_count; ++i) {
                    post_wakeup();
                }
        ...
        void restart() noexcept override {
            stopped_.store(false, core::layer::memory_order_release);
        }

        bool stopped() const noexcept override {
            return stopped_.load(core::layer::memory_order_acquire);
        }
        ...
        std::size_t run() override {
            ...
            while (!stopped_.load(core::layer::memory_order_acquire)) {
                if (work_count_.load(core::layer::memory_order_acquire) <= 0) {
                    break;
                }
        ...
        std::size_t run_one() override {
            in_event_loop_ = true;
            std::size_t count = 0;
            if (!stopped_.load(core::layer::memory_order_acquire)) {
                if (work_count_.load(core::layer::memory_order_acquire) > 0) {
...
namespace rainy::foundation::io::implements {
    core::memory::nebula_ptr<io_context_impl_base> create_io_context_impl(int concurrency_hint) {
        auto impl = core::memory::make_nebula<win_iocp_impl>(concurrency_hint);
