...
namespace rainy::foundation::io::filesystem {
    ...
    path absolute(const path &path, std::error_code &ec) {
        ...
        core::text::wstring_view s = path.native();
        ...
        do {
            buf.resize_and_overwrite(len, [&s, &len, &path](wchar_t *p, unsigned n) -> std::int32_t {
                len = core::layer::absolute_native(s.data(), p, n);
                return len == -1 ? 0 : len;
            });
    ...
    path canonical(const path &path, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&s, &len](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::canonical_native(s.data(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == -1) {
    ...
    void copy(const path &from, const path &to, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::copy_native(from.native().c_str(), to.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void copy(const path &from, const path &to, const copy_options options, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::copy_native(from.native().c_str(), to.native().c_str(), options);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    bool copy_file(const path &from, const path &to, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::copy_file_native(from.native().c_str(), to.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
            return false;
        }
    ...
    bool copy_file(const path &from, const path &to, const copy_options option, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::copy_file_native(from.native().c_str(), to.native().c_str(), option);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
            return false;
        }
    ...
    void copy_symlink(const path &existing_symlink, const path &new_symlink, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::copy_symlink_native(existing_symlink.native().c_str(), new_symlink.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    bool create_directories(const path &path, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::create_directories_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return success;
    ...
    bool create_directory(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::create_directory_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return success;
    ...
    bool create_directory(const path &path, const filesystem::path &existing_p, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::create_directory_native(path.native().c_str(), existing_p.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return success;
    ...
    void create_directory_symlink(const path &to, const path &new_symlink, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::create_directory_symlink_native(to.native().c_str(), new_symlink.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void create_hard_link(const path &to, const path &new_hard_link, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::create_hard_link_native(to.native().c_str(), new_hard_link.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void create_symlink(const path &to, const path &new_symlink, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::create_symlink_native(to.native().c_str(), new_symlink.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    path current_path(std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::current_path_native(p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    void current_path(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::current_path_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    bool equivalent(const path &left, const path &right, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool success = core::layer::equivalent_native(left.native().c_str(), right.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return success;
    ...
    bool exists(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool exsits = core::layer::exists_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
            return false;
        }
    ...
    std::uintmax_t file_size(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto size = core::layer::file_size_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
            return -1;
        }
    ...
    bool is_block_file(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_block = core::layer::is_block_file_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_block;
    ...
    bool is_character_file(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_character = core::layer::is_character_file_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_character;
    ...
    bool is_directory(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_directory = core::layer::is_directory_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_directory;
    ...
    bool is_empty(const path &path, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_empty = core::layer::is_empty_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_empty;
    ...
    bool is_fifo(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_fifo = core::layer::is_fifo_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_fifo;
    ...
    bool is_other(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_fifo = core::layer::is_other_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_fifo;
    ...
    bool is_regular_file(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_regular = core::layer::is_regular_file_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_regular;
    ...
    bool is_socket(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_socket = core::layer::is_socket_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_socket;
    ...
    bool is_symlink(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool is_symlink = core::layer::is_symlink_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return is_symlink;
    ...
    file_time_type last_write_time(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto raw = core::layer::last_write_time_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    ...
    void last_write_time(const path &path, const file_time_type new_time, std::error_code &ec) noexcept {
        ...
        core::layer::last_write_time_native(path.native().c_str(), t);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void permissions(const path &path, const perms prms, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::permissions_native(path.native().c_str(), prms);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void permissions(const path &path, const perms prms, const perm_options opts, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::permissions_native(path.native().c_str(), prms, opts);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    path proximate(const path &path, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&path, &len](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::proximate_native(path.native().data(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    path read_symlink(const path &path, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len, &path](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::read_symlink_native(path.native().c_str(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    path relative(const path &path, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len, &path](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::relative_native(path.native().c_str(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    path relative(const path &path, const filesystem::path &base, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len, &path, &base](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::relative_native(path.native().c_str(), base.native().c_str(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    bool remove(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const bool removed = core::layer::remove_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return removed;
    ...
    std::uintmax_t remove_all(const path &path, std::error_code &ec) {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto count = core::layer::remove_all_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
        return count;
    ...
    void rename(const path &from, const path &to, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::rename_native(from.native().c_str(), to.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    void resize_file(const path &path, const std::uintmax_t size, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        core::layer::resize_file_native(path.native().c_str(), size);
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
        }
    }
    ...
    space_info space(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto [capacity, free, available] = core::layer::space_native(path.native().c_str());
        space_info result = {};
    ...
    file_status status(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto [type, permissions] = core::layer::status_native(path.native().c_str());
        if (errno != 0) {
    ...
    file_status symlink_status(const path &path, std::error_code &ec) noexcept {
        RAINY_FILESYSTEM_INIT_SYSCALL(ec);
        const auto [type, permissions] = core::layer::symlink_status_native(path.native().c_str());
        if (errno != 0) {
            ec = std::error_code(errno, std::system_category());
            return {};
        }
    ...
    path temp_directory_path(std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::temp_directory_path_native(p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
    ...
    path weakly_canonical(const path &path, std::error_code &ec) {
        ...
        do {
            buf.resize_and_overwrite(len, [&len, &path](auto *p, unsigned n) -> std::int32_t {
                len = core::layer::weakly_canonical_native(path.c_str(), p, n);
                return len == -1 ? 0 : len;
            });
        } while (len > buf.size() && len != -1);
        if (len == 0) {
