...
namespace rainy::foundation::io::filesystem {
    ...
    directory_iterator::directory_iterator(const filesystem::path &path) : dir_(core::memory::make_shared<dir_impl>()) {
        std::error_code ec;
        dir_ = core::memory::make_shared<dir_impl>(path, directory_options::none, ec);
        if (ec || !dir_->is_valid) {
            dir_.reset();
        }
    }
    ...
    directory_iterator::directory_iterator(const filesystem::path &path, directory_options options) :
        dir_(core::memory::make_shared<dir_impl>()) {
        std::error_code ec;
        dir_ = core::memory::make_shared<dir_impl>(path, options, ec);
        if (ec || !dir_->is_valid) {
            dir_.reset();
        }
    }
    ...
    directory_iterator::directory_iterator(const path &p, directory_options opts, std::error_code *ec) : dir_() {
        if (ec != nullptr) {
            dir_ = core::memory::make_shared<dir_impl>(p, opts, *ec);
            if (*ec || !dir_->is_valid) {
                dir_.reset();
            }
        } else {
            std::error_code local_ec;
            dir_ = core::memory::make_shared<dir_impl>(p, opts, local_ec);
    ...
    recursive_directory_iterator::recursive_directory_iterator(const filesystem::path &path) : dirs(core::memory::make_shared<dir_stack>()) {
        std::error_code ec;
        dirs = core::memory::make_shared<dir_stack>(path, directory_options::none, ec);
        if (ec || !dirs->is_valid) {
            dirs.reset();
        }
    }
    ...
    recursive_directory_iterator::recursive_directory_iterator(const filesystem::path &path, directory_options options) :
        dirs(core::memory::make_shared<dir_stack>()) {
        std::error_code ec;
        dirs = core::memory::make_shared<dir_stack>(path, options, ec);
        if (ec || !dirs->is_valid) {
            dirs.reset();
        }
    }
    ...
    recursive_directory_iterator::recursive_directory_iterator(const path &p, directory_options opts, std::error_code *ec) : dirs() {
        if (ec != nullptr) {
            dirs = core::memory::make_shared<dir_stack>(p, opts, *ec);
            if (*ec || !dirs->is_valid) {
                dirs.reset();
            }
        } else {
            std::error_code local_ec;
            dirs = core::memory::make_shared<dir_stack>(p, opts, local_ec);
