...
namespace rainy::foundation::io::filesystem::implements {
    ...
    public:
        ...
        std::error_code open(const filesystem::path &path, const open_mode mode, io_context::executor_type executor) noexcept override {
            int flags = 0;
            // NOLINTBEGIN
            ...
            if (rainy_const res = uring_file_proxy{executor}.associate_handle(nullptr, static_cast<std::uintptr_t>(fd_), nullptr);
                res != concurrency::implements::thrd_result::success) {
                ::close(fd_);
                fd_ = -1;
    ...
    core::memory::unique_ptr<file_impl_base> make_file_impl() {
        return core::memory::make_unique<uring_file_impl>();
    }
}
