...
namespace rainy::foundation::io::filesystem {
    ...
    directory_entry::directory_entry(filesystem::path path) : pathobject(utility::move(path)) {
        refresh();
    }

    directory_entry::directory_entry(filesystem::path path, std::error_code &ec) : pathobject(utility::move(path)) {
        refresh(ec);
    }
