...
namespace rainy::foundation::exceptions::runtime {
    class filesystem_error : public core::exceptions::runtime::system_error {
    public:
        using path = foundation::io::filesystem::path;

        filesystem_error(const core::text::string &what_arg, const std::error_code ec,
                         const source &location = core::diagnostics::source_location::current()) : system_error(ec, what_arg, location) {
        }

        filesystem_error(const core::text::string &what_arg, path p1, const std::error_code ec,
                         const source &location = core::diagnostics::source_location::current()) :
            system_error(ec, what_arg, location), path1_(utility::move(p1)) {
        }

        filesystem_error(const core::text::string &what_arg, path p1, path p2, const std::error_code ec,
                         const source &location = core::diagnostics::source_location::current()) :
