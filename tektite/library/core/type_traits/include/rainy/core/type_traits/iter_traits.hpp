...
namespace rainy::type_traits::extras::iterators {
    ...
    template <typename Ty>
    struct iterator {
        using type = type_traits::modifers::remove_reference_t<decltype(utility::declval<Ty &>().begin())>;
    };
    ...
    struct const_iterator {
        using type = type_traits::modifers::remove_reference_t<decltype(utility::declval<const Ty &>().cbegin())>;
    };
