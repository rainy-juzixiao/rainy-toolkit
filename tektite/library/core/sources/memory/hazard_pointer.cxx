...
namespace rainy::core::memory::implements {
    ...
    hazard_pointer_registry::~hazard_pointer_registry() {
        // 结束时，清除所有hazard链表
        if (thread_count_.load(concurrency::memory_order_acquire) == 0) {
            thread_hazard_list *curr = head_.load(concurrency::memory_order_acquire);
            while (curr) {
        ...
        if (thread_list == nullptr) {
            ...
            while (curr) {
                if (bool expected = false;
                    curr->active.compare_exchange_strong(expected, true, concurrency::memory_order_acquire, concurrency::memory_order_relaxed)) {
                    thread_list = curr;
                    thread_count_.fetch_add(1, concurrency::memory_order_relaxed);
                    break;
                }
                curr = curr->next;
            }
            if (thread_list == nullptr) {
                ...
                thread_hazard_list *old_head = head_.load(concurrency::memory_order_relaxed);
                do {
                    thread_list->next = old_head;
                } while (!head_.compare_exchange_weak(old_head, thread_list, concurrency::memory_order_release, concurrency::memory_order_relaxed));
                thread_count_.fetch_add(1, concurrency::memory_order_relaxed);
            }
    ...
    void hazard_pointer_registry::mark_inactive() {
        ...
        if (thread_list) {
            for (auto &h: thread_list->hazards) {
                h.store(nullptr, concurrency::memory_order_release);
            }
            thread_list->active.store(false, concurrency::memory_order_release);
            thread_count_.fetch_sub(1, concurrency::memory_order_relaxed);
        }
    }

    collections::vector<void *> hazard_pointer_registry::scan_hazard_pointers() const {
        collections::vector<void *> protected_ptrs;
        protected_ptrs.reserve(256);
        const void *SLOT_OWNED_MARKER = reinterpret_cast<void *>(0x1);
        thread_hazard_list *curr = head_.load(concurrency::memory_order_acquire);
        while (curr) {
            if (const bool is_active = curr->active.load(concurrency::memory_order_acquire); is_active) {
                for (auto &hazard: curr->hazards) {
                    if (void *ptr = hazard.load(concurrency::memory_order_acquire); ptr != nullptr && ptr != SLOT_OWNED_MARKER) {
    ...
    std::size_t hazard_pointer_registry::get_active_thread_count() const {
        return thread_count_.load(concurrency::memory_order_relaxed);
    }
    ...
    void global_reclamation_manager::add_orphaned_nodes(retire_node *head, const std::size_t count) {
        ...
        retire_node *old_head = head_.load(concurrency::memory_order_relaxed);
        // NOLINTBEGIN
        do {
            tail->next = old_head;
        } while (!head_.compare_exchange_weak(old_head, head, concurrency::memory_order_release, concurrency::memory_order_relaxed));
        // NOLINTEND
        count_.fetch_add(count, concurrency::memory_order_relaxed);
    }

    std::size_t global_reclamation_manager::reclaim(const collections::vector<void *> &protected_ptrs) {
        retire_node *curr = head_.exchange(nullptr, concurrency::memory_order_acquire);
        if (!curr) {
            return 0;
        }
        count_.store(0, concurrency::memory_order_relaxed);
    ...
    std::size_t global_reclamation_manager::get_orphaned_count() const {
        return count_.load(concurrency::memory_order_relaxed);
    }
}
...
namespace rainy::core::memory {
    hazard_pointer::hazard_pointer() : slot_(nullptr), slot_index_(0) {
        ...
        for (std::size_t i = 0; i < implements::thread_hazard_list::MAX_HAZARDS_PER_THREAD; ++i) {
            ...
            if (void *expected = nullptr; thread_list->hazards[i].compare_exchange_strong(
                    expected, const_cast<void *>(SLOT_OWNED_MARKER), concurrency::memory_order_acquire, concurrency::memory_order_relaxed)) {
    ...
    bool hazard_pointer::is_protected() const noexcept {
        ...
        const void *ptr = slot_->load(concurrency::memory_order_acquire);
    ...
    void *hazard_pointer::get_protected() const noexcept {
        ...
        void *ptr = slot_->load(concurrency::memory_order_acquire);
