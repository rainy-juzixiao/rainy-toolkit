...
namespace rainy::core::layer {
    ...
    namespace {
        struct the_task {
            ...
            pid_t tid{0};
#endif
            unsigned int (*invoke_address)(void *) = {};
...
#endif
            core::layer::interlocked_exchange(&task->tid_ready, 0xA0);
        }
        return execute_task(arg_list);
    }

    schd_thread_t create_thread(unsigned int (*invoke_function_addr)(void *),
        ...
        while (iso_volatile_load(&task.tid_ready) == 0) {
            thread_yield();
        }
#if RAINY_USING_MACOS
