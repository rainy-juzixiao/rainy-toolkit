...
namespace rainy::core::layer {
    ...
    DWORD timespec_to_ms_remaining(const ::timespec *timeout) noexcept {
        ...
        auto deadline = system_clock::time_point(duration_cast<system_clock::duration>(ns));
    ...
    thrd_result smtx_create(smtx_t *const smtx) noexcept {
        ...
        *smtx = allocate(sizeof(shared_mutex_handle), alignof(shared_mutex_handle));
        if (*smtx == nullptr) {
            return thrd_result::nomem;
        }
    ...
    thrd_result smtx_destroy(smtx_t *const smtx) noexcept {
        ...
        auto *handle = static_cast<shared_mutex_handle *>(*smtx);
        deallocate(handle, sizeof(shared_mutex_handle), alignof(shared_mutex_handle));
        *smtx = nullptr;
        return thrd_result::success;
    }
    ...
    thrd_result smtx_timed_lock(smtx_t *const smtx, const ::timespec *timeout) noexcept {
        ...
        while (true) {
            ...
            WaitOnAddress(const_cast<LONG *>(&handle->state), &observed, sizeof(LONG), ms);
        }
    }
