...
#if RAINY_USING_MACOS
        return interlocked_and64(reinterpret_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#else
        return interlocked_and64(static_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#endif

#else

#if RAINY_USING_MACOS
        return interlocked_and32(reinterpret_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#else
        return interlocked_and32(static_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#endif
...
#if RAINY_USING_MACOS
        return interlocked_or64(reinterpret_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#else
        return interlocked_or64(static_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#endif

#else

#if RAINY_USING_MACOS
        return interlocked_or32(reinterpret_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#else
        return interlocked_or32(static_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#endif
...
#if RAINY_USING_MACOS
        return interlocked_xor64(reinterpret_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#else
        return interlocked_xor64(static_cast<volatile std::int64_t *>(value), static_cast<const std::int64_t>(mask));
#endif

#else

#if RAINY_USING_MACOS
        return interlocked_xor32(reinterpret_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#else
        return interlocked_xor32(static_cast<volatile std::int32_t *>(value), static_cast<const std::int32_t>(mask));
#endif
...
#if RAINY_USING_MACOS
        iso_volatile_store64(reinterpret_cast<volatile std::int64_t *>(address), *static_cast<std::int64_t *>(value));
#else
...
#if RAINY_USING_MACOS
        iso_volatile_store32(reinterpret_cast<volatile std::int32_t *>(address), *static_cast<std::int32_t *>(value));
#else
...
namespace rainy::core::layer {
    void atomic_thread_fence(const memory_order order) noexcept {
        ...
        if (order == memory_order_seq_cst) {
            ...
            core::layer::interlocked_increment(&guard);
            rainy_compiler_barrier();
        }
#elif RAINY_IS_ARM64
        // ARM 平台
...
namespace rainy::core::layer::implements {
