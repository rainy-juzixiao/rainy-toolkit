...
namespace rainy::type_traits::properties::implements {
    ...
    template <>
    struct is_destructible<void> : helper::false_type {};

    template <typename Ret, typename... Args>
    struct is_destructible<Ret(Args...)> : helper::false_type {};
    ...
    template <typename Ty>
    struct is_destructible<Ty[]> : is_destructible<Ty> {};
...
namespace rainy::type_traits::properties {
     ...
     *
     * \lang simp-chinese
     * @brief 检查 From 类型是否可以转换为 To 类型用于调用操作的主模板。
     *         默认为 false_type。
     *
     * @tparam From 源类型
     * @tparam To 目标类型
    ...
    struct is_invoke_convertible<From, To,
                                 other_trans::void_t<decltype(rainy::type_traits::implements::fake_copy_init<To>(
                                     rainy::type_traits::implements::returns_exactly<From>()))>> : helper::true_type {};
     ...
     *
     * \lang simp-chinese
     * @brief 检查 From 到 To 的转换对于调用操作是否为 noexcept。
     *
     * @tparam From 源类型
     * @tparam To 目标类型
     */
    template <typename From, typename To>
    struct is_invoke_nothrow_convertible : helper::bool_constant<noexcept(rainy::type_traits::implements::fake_copy_init<To>(
                                               rainy::type_traits::implements::returns_exactly<From>()))> {};
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 和 Uy 是否可以相互交换。
     *
     * @tparam Ty 第一个类型
     * @tparam Uy 第二个类型
     */
    template <typename Ty, typename Uy>
    struct is_swappable_with : type_traits::implements::swap_adl::is_swappable_with_impl<Ty, Uy> {};
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 和 Uy 是否可以相互交换且交换操作是 noexcept。
     *
     * @tparam Ty 第一个类型
     * @tparam Uy 第二个类型
     */
    ...
    struct is_nothrow_swappable_with
        : helper::bool_constant<is_swappable_with_v<Ty, Uy> &&
                                type_traits::implements::swap_adl::is_nothrow_swappable_with_impl<Ty, Uy>::value> {};
     ...
     *
     * \lang simp-chinese
     * @brief nothrow_swappable_with 检查的变量模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 是否可以与自身交换。
     *         使用 Ty 的左值引用进行检查。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    struct is_swappable : is_swappable_with<modifers::add_lvalue_reference_t<Ty>, modifers::add_lvalue_reference_t<Ty>> {};
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 是否可以与自身交换且交换操作是 noexcept。
     *         使用 Ty 的左值引用进行检查。
     *
     * @tparam Ty 要检查的类型
     */
    ...
    struct is_nothrow_swappable
        : is_nothrow_swappable_with<modifers::add_lvalue_reference_t<Ty>, modifers::add_lvalue_reference_t<Ty>> {};
     ...
     *
     * \lang simp-chinese
     * @brief nothrow_swappable 检查的变量模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 是否有 ADL（参数依赖查找）swap 的变量模板。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型 Ty 是否有 ADL swap 的类型模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 From 到 To 的赋值是否可平凡赋值的变量模板。
    ...
    RAINY_CONSTEXPR_BOOL is_trivially_assignable_v = __is_trivially_assignable(modifers::add_lvalue_reference_t<To>, From);
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 From 到 To 的赋值是否可平凡赋值的类型模板。
     *         使用编译器内建 __is_trivially_assignable。
     *
     * @tparam To 目标类型
     * @tparam From 源类型
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可平凡移动赋值的变量模板。
     *         使用编译器内建 __is_trivially_assignable。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_trivially_move_assignable_v = __is_trivially_assignable(modifers::add_lvalue_reference_t<Ty>, Ty);
    ...
    RAINY_CONSTEXPR_BOOL is_trivially_copy_assignable_v =
        __is_trivially_assignable(modifers::add_lvalue_reference_t<Ty>, modifers::add_lvalue_reference_t<const Ty>);
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可移动赋值的变量模板。
     *         使用编译器内建 __is_assignable。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    constexpr bool is_move_assignable_v = __is_assignable(modifers::add_lvalue_reference_t<Ty>, Ty);
    ...
    constexpr bool is_copy_assignable_v =
        __is_assignable(modifers::add_lvalue_reference_t<Ty>, modifers::add_lvalue_reference_t<const Ty>);
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为 POD（Plain Old Data）类型的变量模板。
     *         POD 类型满足标准布局且是平凡类型。
     *
     * @tparam Ty 要查询的类型
     */
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为 POD（Plain Old Data）类型的类型模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为有符号类型的变量模板。
     *         使用 implements::sign_base 判断是否有符号。
     *
     * @tparam Ty 要查询的类型
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_signed_v = rainy::type_traits::implements::sign_base<Ty>::is_signed;
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为无符号类型的变量模板。
     *         使用 implements::sign_base 判断是否无符号。
     *
     * @tparam Ty 要查询的类型
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_unsigned_v = rainy::type_traits::implements::sign_base<Ty>::is_unsigned;
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可复制构造的变量模板。
     *         检查从 const 左值引用构造。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_copy_constructible_v = __is_constructible(Ty, modifers::add_lvalue_reference_t<const Ty>);
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 'from' 到 'to' 的赋值是否可能的变量模板。
     *         使用编译器内建 __is_assignable。
     *
     * @tparam to 目标类型（作为左值引用）
     * @tparam from 源类型
     */
    template <typename to, typename from>
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 'from' 到 'to' 的赋值是否可能的类型模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可以使用给定参数无异常构造的变量模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可以使用给定参数无异常构造的类型模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 'from' 到 'to' 的赋值是否无异常的变量模板。
     ...
     */
    template <typename to, typename from>
    RAINY_CONSTEXPR_BOOL is_nothrow_assignable_v = rainy::type_traits::implements::is_nothrow_assignable_v<to, from>;
     ...
     *
     * \lang simp-chinese
     * @brief 检查从 'from' 到 'to' 的赋值是否无异常的类型模板。
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可无异常复制构造的变量模板。
     *         检查从 const 左值引用进行的不抛异常的复制构造。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    constexpr bool is_nothrow_copy_constructible_v = __is_nothrow_constructible(Ty, modifers::add_lvalue_reference_t<const Ty>);
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否可无异常移动赋值的变量模板。
     *         使用编译器内建 __is_nothrow_assignable。
     *
     * @tparam Ty 要检查的类型
     */
    template <typename Ty>
    constexpr bool is_nothrow_move_assignable_v = __is_nothrow_assignable(modifers::add_lvalue_reference_t<Ty>, Ty);
    ...
    constexpr bool is_nothrow_copy_assignable_v =
        __is_nothrow_assignable(modifers::add_lvalue_reference_t<Ty>, modifers::add_lvalue_reference_t<const Ty>);
     ...
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_destructible_v = implements::is_destructible<Ty>::value;
     ...
     */
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL is_nothrow_destructible_v = std::is_nothrow_destructible_v<Ty>;
    ...
    RAINY_CONSTEXPR_BOOL is_trivially_copy_constructible_v =
        is_trivially_constructible_v<Ty, modifers::add_lvalue_reference_t<const Ty>>;
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为有作用域枚举的变量模板。
     *         主模板默认为 false。
     *
     * @tparam Ty 要检查的类型
     * @tparam is_enum 指示 Ty 是否为枚举的布尔值
     */
     ...
     *
     * \lang simp-chinese
     * @brief 确定类型是否应通过值传递的变量模板。
     *         非类、非联合、非数组、非函数类型的主模板。
     *         大小小于等于2个指针且可平凡复制的类型优先通过值传递。
     *
     * @tparam Ty 要检查的类型
     * @tparam is_special 指示 Ty 是否为类/联合/数组/函数类型的布尔值
     */
...
namespace rainy::type_traits::properties {
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为 volatile 限定的变量模板。
     *
     * @tparam Ty 要查询的类型
     */
    template <typename Ty>
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为 volatile 限定的类型模板。
     *
     * @tparam Ty 要查询的类型
     */
    template <typename Ty>
     ...
     *
     * \lang simp-chinese
     * @brief 检查类型是否为平凡类型的类型模板。
     *
     * @tparam Ty 要查询的类型
     * @remark 如果类型 Ty 是平凡类型，则实例为 true；否则为 false。
     *          平凡类型是标量类型、完全可复制类类型、这些类型的数组
     *          以及这些类型的 cv 限定版本。
     */
    template <typename Ty>
