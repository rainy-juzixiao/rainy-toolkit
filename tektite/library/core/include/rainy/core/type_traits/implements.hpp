...
namespace rainy::type_traits::implements {
    ...
    template <typename Ty>
    using remove_cvref_t = remove_cv_t<modifers::remove_reference_t<Ty>>;
...
namespace rainy::utility {
     ...
     */
    template <typename Ty>
    RAINY_NODISCARD constexpr rain_fn move(Ty &&arg) noexcept -> type_traits::modifers::remove_reference_t<Ty> && {
...
namespace rainy::utility {
     ...
     *
     * \lang simp-chinese
     * @brief 有条件地将对象转换为右值引用，如果移动操作可能抛出异常则回退到常量左值引用。
     *
     * @tparam Ty 要转换的对象的类型。
     * @param arg 要转换的对象。
     * @return 如果 `Ty` 是不抛异常的移动可构造，则返回右值引用（`Ty&&`）；
...
namespace rainy::utility {
     ...
     *
     * \lang simp-chinese
     * @brief 在给定的内存位置就地构造对象。
     *
     * @tparam Ty 要构造的对象类型
     * @tparam Args 构造函数参数类型
     * @param object 指向将要构造对象的内存位置的引用
     * @param args 要转发的构造函数参数
     * @throws 如果 Ty 的构造函数是 noexcept 则不抛出异常
     */
    template <typename Ty, typename... Args>
     ...
     *
     * \lang simp-chinese
     * @brief 销毁从 first 到 last 范围内的对象。
     *
     * @tparam Ty 要销毁的对象类型
     * @param first 指向范围内第一个对象的指针
     * @param last 指向范围内最后一个对象之后位置的指针
     * @throws 始终不抛出异常
     */
    template <typename Ty>
     ...
     *
     * \lang simp-chinese
     * @brief 销毁范围内的对象（destroy_range 的别名）。
     *
     * @tparam Ty 要销毁的对象类型
     * @param first 指向范围内第一个对象的指针
     * @param last 指向范围内最后一个对象之后位置的指针
