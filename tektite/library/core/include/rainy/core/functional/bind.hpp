...
#define RAINY_CORE_FUNCTIONAL_BIND_HPP
...
namespace rainy::functional::implements {
    ...
    struct binder_arg<Ty, type_traits::other_trans::enable_if_t<is_binder_v<Ty>>> {
        ...
        static constexpr decltype(auto) adjust(UTy &&binder_obj, CallTuple &call_args) {
            return apply_binder(
                utility::forward<UTy>(binder_obj), call_args,
                type_traits::helper::make_index_sequence<std::tuple_size_v<type_traits::modifers::remove_reference_t<CallTuple>>>{});
        }
    };
    ...
    constexpr rain_fn call_binder_impl(Fx &f, BoundTuple &bound, CallTuple &&call, type_traits::helper::index_sequence<I...>) noexcept(
        noexcept(utility::invoke(
            utility::declval<Fx &>(),
            binder_arg<type_traits::modifers::remove_cvref_t<decltype(utility::declval<BoundTuple &>().template get<I>())>>::adjust(
                utility::declval<BoundTuple &>().template get<I>(), utility::declval<CallTuple &>())...))) -> decltype(auto) {
        return utility::invoke(f, binder_arg<type_traits::modifers::remove_cvref_t<decltype(bound.template get<I>())>>::adjust(
...
namespace rainy::functional {
    ...
    public:
        ...
        constexpr decltype(auto) operator()(CallArgs &&...args) noexcept(
            implements::is_nothrow_invocable_of_call_binder<func_type, bound_args, container::tuple<CallArgs...>, impl_bound_seq>) {
            auto &func = pair.get_first();
            auto &bound = pair.get_second();
            return implements::call_binder_impl(func, bound, container::forward_as_tuple(utility::forward<CallArgs>(args)...),
                                                impl_bound_seq{});
        }
#else
        template <typename... CallArgs,
                  type_traits::other_trans::enable_if_t<
                      implements::can_call_binder<func_type, bound_args, container::tuple<CallArgs...>, impl_bound_seq>, int> = 0>
        constexpr decltype(auto) operator()(CallArgs &&...args) noexcept(
            implements::is_nothrow_invocable_of_call_binder<func_type, bound_args, container::tuple<CallArgs...>, impl_bound_seq>) {
            auto &func = pair.get_first();
            auto &bound = pair.get_second();
