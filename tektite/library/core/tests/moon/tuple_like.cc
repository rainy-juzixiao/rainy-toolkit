...
TEST_CASE("member_names", "[moon][member_names]") {
    SECTION("aggregate struct names") {
        constexpr auto names = member_names<SimpleStruct>();
    ...
    SECTION("registered struct names") {
        constexpr auto names = member_names<RegisteredStruct>();
    ...
    SECTION("private reflected struct names") {
        constexpr auto names = member_names<PrivateClass>();
    ...
    SECTION("empty struct names") {
        constexpr auto names = member_names<EmptyStruct>();
...
TEST_CASE("for_each", "[moon][for_each]") {
    SECTION("for_each over type - names only") {
        std::vector<rainy::core::text::string_view> captured;

        for_each<SimpleStruct>([&](rainy::core::text::string_view name, std::size_t idx) {
    ...
    SECTION("for_each over object") {
        ...
        for_each(obj, [&](auto &member, rainy::core::text::string_view name, std::size_t idx) { values.push_back(static_cast<int>(member)); });
...
TEST_CASE("tuple and pair traits", "[moon][tuple_traits][pair_traits]") {
    ...
    SECTION("is_pair") {
        ...
        REQUIRE(is_pair_v<rainy::container::pair<int, double>>);
...
TEST_CASE("pair reflection specialization", "[moon][pair]") {
    SECTION("std::pair") {
        ...
        constexpr auto names = member_names<std::pair<int, double>>();
...
TEMPLATE_TEST_CASE("template struct reflection", "[moon][template]", int, double, std::string) {
    ...
    constexpr auto names = member_names<Struct>();
    REQUIRE(names.size() == 2);
...
TEST_CASE("compile-time constexpr evaluation", "[moon][constexpr]") {
    ...
    SECTION("member_names is consteval") {
        constexpr auto names = member_names<SimpleStruct>();
...
struct rainy::core::meta::reflectet_for_type<ManuallyRegistered> {
    ...
    static constexpr auto member_names() noexcept {
        rainy::collections::array<rainy::core::text::string_view, 3> names = {"field1", "field2", "field3"};
        return names;
    }
};
...
TEST_CASE("manual registration with RAINY_REFLECT_TUPLE_LIKE", "[moon][manual_registration]") {
    ...
    constexpr auto names = member_names<ManuallyRegistered>();
    REQUIRE(names.size() == 3);
...
TEST_CASE("C++26 static reflection - annotations", "[moon][cxx26][annotations]") {
    SECTION("annotation presence") {
        constexpr auto names = member_names<AnnotatedForTest>();
    ...
    SECTION("prefix and suffix tags") {
        ...
        for (const auto &name: names) {
            if (name.find("pre_") != rainy::core::text::string_view::npos && name.find("_suf") != rainy::core::text::string_view::npos) {
                found_prefixed = true;
            }
        }
    ...
    SECTION("type-level annotations") {
        constexpr auto names = member_names<TypeAnnotatedStruct>();
        for (const auto &name: names) {
            // 应该包含类型级别的前缀和后缀
            REQUIRE(name.find("type_") != rainy::core::text::string_view::npos);
            REQUIRE(name.find("_type") != rainy::core::text::string_view::npos);
        }
    }
}
