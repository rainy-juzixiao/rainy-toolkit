...
namespace rainy::foundation::type_traits::internals {
    template <typename...>
    using void_t = void;
...
/* 数组修改 */
namespace rainy::foundation::type_traits::array_modify {
    template <typename Ty>
...
// 引用修改
namespace rainy::foundation::type_traits::reference_modify {
    template <typename Ty>
...
// const-volatile 修改
namespace rainy::foundation::type_traits::cv_modify {
    /**
     * @brief 从类型设置常量类型
     * @tparam Ty 要修改的类型
     */
...
namespace rainy::foundation::type_traits::other_transformations {
    /**
    ...
    using conditional_t = typename conditional<test, if_true, if_false>::type;
...
namespace rainy::foundation::type_traits::internals {
    ...
    template <typename>
    RAINY_CONSTEXPR_BOOL _is_function_v = false;

    template <typename Rx, typename... Args>
    RAINY_CONSTEXPR_BOOL _is_function_v<Rx(Args...)> = true;
    ...
    struct invoker_functor {
        ...
        static constexpr auto invoke(callable &&object,
                                     Args &&...args) noexcept(noexcept(static_cast<callable &&>(object)(static_cast<Args &&>(args)...)))
    ...
    struct invoker_pmf_object {
        ...
        static constexpr auto invoke(decayed pmf, Ty &&args1,
    ...
    template <typename>
    struct is_member_function_pointer_ {
        static constexpr bool value = false;
    };

    template <typename Ty, typename Class>
    struct is_member_function_pointer_<Ty Class::*> {
        static constexpr bool value = _is_function_v<Ty>;
    };
    ...
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL _is_member_pointer_v = __is_member_pointer(Ty);
#else
    template <typename Ty>
    RAINY_CONSTEXPR_BOOL _is_member_pointer_v = is_member_object_pointer_<Ty>::value || is_member_function_pointer_<Ty>::value;
#endif // RAINY_USING_CLANG

    template <typename Ty>
    struct decay_ { // determines decayed version of Ty
        using Ty1 = remove_reference_t_<Ty>;
        using Ty2 = typename _select<_is_function_v<Ty1>>::template apply<add_pointer_<Ty1>, remove_cv<Ty1>>;
        using type = typename _select<_is_array_v<Ty1>>::template apply<add_pointer_<array_modify::remove_extent_t<Ty1>>, Ty2>::type;
    };

    template <typename Ty>
    using decay_t_ = typename decay_<Ty>::type;
...
/* 指针修改 */
namespace rainy::foundation::type_traits::pointer_modify {
    template <typename Ty>
...
namespace rainy::foundation::type_traits::helper {
    /**
     * @brief 从类型和值生成整型常量。
     * @tparam Ty 类型
     * @tparam data 值
...
// 类型关系
namespace rainy::foundation::type_traits::type_relations {
    /**
     * @brief 测试两个类型是否相同
     * @tparam Ty1 检索的第一个类型
     * @tparam Ty2 检索的第二个类型
...
namespace rainy::foundation::type_traits::logical_traits {
    template <typename...>
...
namespace rainy::utility {
    ...
    template <typename Ty>
    foundation::type_traits::internals::add_rvalue_reference_t<Ty> declval() noexcept {
        static_assert(foundation::type_traits::internals::always_false<Ty>, "Calling declval is ill-formed, see N4950 [declval]/2.");
        static bool never_returned = true;
        return reinterpret_cast<foundation::type_traits::internals::add_rvalue_reference_t<Ty>>(never_returned);
    }
...
namespace rainy::foundation::type_traits::type_properties {
    ...
    struct is_invocable_r {
        template <typename U>
        static auto test(int) -> decltype(static_cast<Rx>(std::declval<U>()(std::declval<Args>()...)), std::true_type{}) {
            return std::true_type{};
        }
...
/* 元方法Trait */
namespace rainy::foundation::type_traits::extras::meta_method {
    template <typename Ty>
    struct has_iterator {
        template <typename U>
...
namespace rainy::utility {
     ...
     */
    template <typename Ty>
    RAINY_NODISCARD constexpr foundation::type_traits::internals::remove_reference_t_<Ty> &&move(Ty &&arg) noexcept {
        return static_cast<typename foundation::type_traits::internals::remove_reference<Ty>::type &&>(arg);
    }
     ...
     */
    template <typename Ty>
    RAINY_NODISCARD constexpr Ty &&forward(foundation::type_traits::internals::remove_reference_t_<Ty> &arg) noexcept {
    ...
    template <typename Ty>
    RAINY_NODISCARD constexpr Ty &&forward(foundation::type_traits::internals::remove_reference_t_<Ty> &&arg) noexcept {
        static_assert(!foundation::type_traits::internals::_is_lvalue_reference_v<Ty>, "bad forward call");
        return static_cast<Ty &&>(arg);
    }
    ...
    constexpr Ty exchange(Ty &val, Other &&new_val) noexcept(foundation::type_traits::internals::_is_nothrow_move_constructible<Ty> &&
                                                             foundation::type_traits::internals::_is_nothrow_assignable_v<Ty &, Other>) {
